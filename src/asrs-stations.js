...
export function appendAsrsStationControls(items,root,layout,onChange){
  for(const item of items.filter(e=>['asrs','stackerCrane'].includes(e.type))){
    ...
    {for(const [key,label,kind] of [['infeedBufferCount','라인별 입고 수용 수량','in'],['outfeedBufferCount','라인별 출고 수용 수량','out'],['stationConveyorSpeed','내부 컨베이어 속도(m/s)',null],['stationSafetyGap','내부 최소 안전간격(m)',null]]){
      const row=document.createElement('label'),input=document.createElement('input');input.type='number';input.min=kind?'1':'.02';input.step=kind?'1':'.01';input.value=item.parameters[key]??(kind?asrsStationSpec(item,layout,kind).count:key==='stationConveyorSpeed'?.5:.2);input.setAttribute('aria-label',label);row.append(label,input);group.append(row);input.addEventListener('change',()=>{if(!input.checkValidity())return;onChange(item,key,Number(input.value));});
    }
    ...
    const note=document.createElement('small');note.textContent='물류 크기·회전 자세·안전간격·양끝 여유로 치수를 자동 계산합니다. 스태커 운반 수량과 버퍼 수량은 별도입니다. 연결점은 각 컨베이어 외측 끝에 표시됩니다. 변경 시 시뮬레이션이 초기화됩니다.';group.append(note);}
    card.append(group);
  }
}
