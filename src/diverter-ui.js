import { diverterDynamics, directionAxis, toggleDiverterDirection, diverterProfile, diverterEnabled, diverterAllowsEdge, diverterEdgeDirection, diverterRouteKey } from './diverter.js';
...
function appendRoutingControls(box,item,layout,onChange){
  ...
  select('디버터 동작 방식',p.diverterMotionMode||'stop',[['stop','정지 후 횡이송'],['moving','이동 중 분기 (고속)']],value=>onChange(item,'diverterMotionMode',value));
  ...
  paragraph(p.diverterMotionMode==='moving'?'이동 중 분기: 사전 수신 승인 시 진입 속도를 이어서 분기합니다. 차단 시 진입 전에 정지하며, 이송 중 차단 시 현재 위치를 유지합니다. 분기 구간은 한 물류만 점유합니다.':'정지 후 횡이송: 하류 수신 승인 → 분기 이송 → 인계 완료. 이송 중 경로를 변경하지 않습니다.');
}
...
export function appendDiverterControls(items,root,layout,onChange){
  for(const item of items.filter(entry=>entry.type==='conveyor')){
    ...
    if(diverterEnabled(item)){
      ...
      const autoRow=document.createElement('label'),auto=document.createElement('input');auto.type='checkbox';auto.checked=Number(p.diverterAutoDynamics)===1;auto.dataset.diverterParameter='diverterAutoDynamics';auto.setAttribute('aria-label','디버터 가감속 자동 계산');autoRow.append('속도·이송거리 기준 가감속 자동 계산',auto);box.append(autoRow);auto.addEventListener('change',()=>onChange(item,'diverterAutoDynamics',auto.checked?1:0));
      const rates=diverterDynamics(item);
      ...
      for(const [key,label,fallback] of [['diverterSpeed','디버터 속도 (m/s)',.5],['diverterAcceleration','가속도 (m/s²)',.8],['diverterDeceleration','감속도 (m/s²)',1],['diverterStroke','디버터 이송거리 (m)',1]]){const row=document.createElement('label'),input=document.createElement('input');input.type='number';input.min='.01';input.step='any';input.value=key==='diverterAcceleration'?rates.acceleration:key==='diverterDeceleration'?rates.deceleration:p[key]??fallback;if(['diverterAcceleration','diverterDeceleration'].includes(key)){input.readOnly=auto.checked;input.title=auto.checked?'자동 계산을 해제하면 수동 입력할 수 있습니다.':'';}input.dataset.diverterParameter=key;input.setAttribute('aria-label',label);row.append(label,input);input.addEventListener('change',()=>{const value=Number(input.value);if(!Number.isFinite(value)||value<=0){input.setCustomValidity('0보다 큰 값을 입력하세요.');input.reportValidity();return;}input.setCustomValidity('');onChange(item,key,value);});box.append(row);}
      const distanceHint=document.createElement('p');distanceHint.textContent='이송거리: 분기 시작부터 하류 인계 완료까지 물류가 분기 방향으로 이동하는 거리입니다. 컨베이어 폭 자체가 아니며 물류 크기·설비 간격·인계 위치에 따라 달라집니다.';box.append(distanceHint);
      appendRoutingControls(box,item,layout,onChange);
      const profile=diverterProfile(item),startY=profile.moving?80-60*profile.startSpeed/Math.max(.000001,profile.peakSpeed):80,endY=profile.moving?80-60*profile.endSpeed/Math.max(.000001,profile.peakSpeed):80,rampY=profile.moving?endY:20,total=profile.total,x1=20+220*profile.t1/total,x2=20+220*(profile.t1+profile.t2)/total;
      ...
      preview.innerHTML=`<strong>분기 흐름 미리보기 · 화면 방향 기준</strong><svg viewBox="0 0 260 130" role="img" aria-label="선택 방향으로 이동하며 물류 자세는 유지"><rect x="85" y="38" width="90" height="54" rx="4" fill="#102c40" stroke="#00d4ff"/><rect x="${130-length*scale/2}" y="${65-width*scale/2}" width="${length*scale}" height="${width*scale}" fill="#ffd166"/>${shownDirections.filter(key=>arrows[key]).map(key=>`<path d="${arrows[key]}" fill="none" stroke="#00ff88" stroke-width="3"/>`).join('')}</svg><strong>디버터 속도 프로파일</strong><svg viewBox="0 0 260 112" role="img" aria-label="디버터 가속 정속 감속 그래프"><path d="M20 15 V80 H245" stroke="#63849a" fill="none"/><path d="M20 ${startY} L${x1} ${rampY} L${x2} ${rampY} L240 ${endY}" stroke="#00d4ff" stroke-width="3" fill="none"/><text x="20" y="105" fill="#d8f3ff" font-size="10">0초 → ${total.toFixed(2)}초 · 최고 ${profile.peakSpeed.toFixed(2)}m/s</text></svg>`;box.append(preview);
      const expand=document.createElement('button');expand.type='button';expand.className='visual-expand-button';expand.dataset.expandDiverter=item.id;expand.textContent='디버터 프로파일 확대 보기';box.append(expand);
      const note=document.createElement('p');note.textContent=`${profile.moving?'속도 조정':'가속'} ${profile.t1.toFixed(2)}초 · 정속 ${profile.t2.toFixed(2)}초 · 감속 ${profile.t3.toFixed(2)}초. ${profile.moving?'이동 중 분기 · 끝에서 정지하지 않음 (사전 승인 조건).':'짧은 이송거리는 삼각형 프로파일입니다.'} 직진은 유지하며 분기 중에는 한 물류만 처리합니다. 물류 자세는 회전하지 않습니다. 디버터 설정을 변경하면 시뮬레이션이 초기화됩니다.`;box.append(note);
