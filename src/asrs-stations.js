...
export function asrsStationSpec(parent,layout,kind,index=null){
  ...
  const count=Math.max(1,Math.round(Number(index==null?null:p.stationLineCounts?.[index]?.[kind])||Number(p[kind==='in'?'infeedBufferCount':'outfeedBufferCount'])||Number(p.retrievalCarryCount)||1));
...
export function appendAsrsStationControls(items,root,layout,onChange){
  for(const item of items.filter(e=>['asrs','stackerCrane'].includes(e.type))){
    ...
    for(let index=0;index<Math.max(1,Number(item.parameters.productTypes)||3);index++){
      ...
      for(const kind of ['in','out']){const spec=asrsStationSpec(item,layout,kind,index),row=document.createElement('label'),input=document.createElement('input');input.type='number';input.min='1';input.step='1';input.value=spec.count;input.setAttribute('aria-label',`${name} ${kind==='in'?'입고':'출고'} 수량`);row.append(`${name} ${kind==='in'?'입고':'출고'} · ${spec.length.toFixed(2)}m`,input);perLine.append(row);input.addEventListener('change',()=>{if(!input.checkValidity())return;const counts=structuredClone(item.parameters.stationLineCounts||{});counts[index]={...counts[index],[kind]:Number(input.value)};onChange(item,'stationLineCounts',counts);});}
    }group.append(perLine);
    const requested=Math.max(...Array.from({length:Math.max(1,Number(item.parameters.productTypes)||3)},(_,index)=>asrsStationSpec(item,layout,'out',index).count)),weight=Number(layout.cargoSpec?.weight)||100,limit=Number(item.parameters.loadCapacity)||1000;
    if(requested*weight>limit){const warning=document.createElement('p');warning.setAttribute('role','status');warning.textContent=`허용 하중 주의: 설정 ${requested}개 × ${weight}kg가 ${limit}kg을 초과합니다. 실제 1회 운반은 최대 ${Math.max(0,Math.floor(limit/weight))}개로 제한됩니다.`;group.append(warning);}
    const note=document.createElement('small');note.textContent='입고는 설정 수량을 받은 뒤 처리하며, 수량 미달 시 가능한 출고를 우선합니다. 출고는 실제 인출 묶음을 동시에 내려놓고 하류에는 설비별 승인으로 전달합니다. 출고 수용 수량이 스태커의 1회 운반 수량입니다. 라인별 개별 설정을 우선하며 재고가 부족하면 있는 수량만 인출합니다. 물류 크기·회전 자세·안전간격·양끝 여유로 치수를 자동 계산합니다. 총 허용 하중을 넘으면 운반 수량이 제한됩니다. 연결점은 각 컨베이어 외측 끝에 표시됩니다. 변경 시 시뮬레이션이 초기화됩니다.';group.append(note);}
    card.append(group);
  }
}
