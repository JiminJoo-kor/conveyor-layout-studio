...
export class LayoutRenderer {
  ...
  drawCadSymbol(item,state){
    ...
    if(['stackerCrane','asrs'].includes(item.type)){const w=compact?60:132,h=compact?48:94,zones=Object.entries(state?.asrs?.zones||{});c.save();c.translate(item.x,item.y);c.strokeStyle=COLORS.green;c.fillStyle='#0d2530';c.lineWidth=1;c.fillRect(-w/2,-h/2,w,h);c.strokeRect(-w/2,-h/2,w,h);zones.forEach(([name,zone],index)=>{const y=-h/2+(index+1)*h/(zones.length+1);c.strokeStyle=COLORS.green;c.beginPath();c.moveTo(-w*.43,y);c.lineTo(w*.43,y);c.stroke();c.fillStyle='#d8f3ff';c.font=`${compact?6:8}px monospace`;c.fillText(`${name.replace(' 라인','')} ${zone.inventory}/${zone.capacity}`,-w*.4,y-3);});c.strokeStyle=COLORS.orange;c.lineWidth=compact?2:3;c.beginPath();c.moveTo(0,-h*.42);c.lineTo(0,h*.42);c.stroke();c.fillStyle=COLORS.orange;c.fillRect(-4,-4,8,8);c.fillStyle='#d8f3ff';c.font=`${compact?7:9}px monospace`;c.fillText('AS/RS',compact?-15:-25,h/2-4);c.restore();return true;}
