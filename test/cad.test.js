...
test('AMR와 AGV의 대기 위치는 주황색 이송 경로의 진행 방향 첫 점이다',()=>{
  for(const type of ['amr','agv']){const before={id:'before',type:'turntable',x:40,y:300},vehicle={id:type,type,x:300,y:120,shuttleRoute:{start:{x:300,y:120},end:{x:500,y:120},points:[{x:300,y:120},{x:500,y:120}]}},after={id:'after',type:'dock',x:600,y:80},incoming={from:'before',to:type,fromPort:'right',toPort:'left',kind:'transfer'},outgoing={from:type,to:'after',fromPort:'right',toPort:'left',kind:'transfer'},layout={equipment:[before,vehicle,after],cadSchematic:{edges:[incoming,outgoing]}},route=mobileEquipmentRoute(layout,vehicle),orangeStart=connectionAnchor(before,incoming.fromPort);assert.equal(route.source,'transfer-path');assert.ok(Math.hypot(route.start.x-before.x,route.start.y-before.y)>Math.hypot(orangeStart.x-before.x,orangeStart.y-before.y));assert.deepEqual(equipmentVisualPosition(layout,vehicle,{t:0,cadTokens:[]}),{x:vehicle.x,y:vehicle.y});assert.deepEqual(equipmentVisualPosition(layout,vehicle,{t:.02,cadTokens:[]}),route.start);}
});
...
test('포킹장치와 AMR은 현장 동작 순서가 보이는 간단 파라미터만 제공한다',()=>{const forkKeys=parameterFieldsFor({type:'forkingDevice',parameters:{jerk:9}}).map(field=>field.key),amr={type:'amr',parameters:{motionProfile:1}},amrFields=parameterFieldsFor(amr),amrKeys=amrFields.map(field=>field.key),angle=amrFields.find(field=>field.key==='cargoTransferAngle');ensureDynamicParameters(amr);assert.ok(['strokeDistance','receiveSpeed','holdTime','transferSpeed'].every(key=>forkKeys.includes(key)));assert.ok(amrKeys.includes('receiveSpeed')&&amrKeys.includes('travelSpeed')&&amrKeys.includes('transferSpeed'));assert.equal(angle.label,'배출 물류 방향(°)');assert.equal(angle.value,0);assert.equal(amr.parameters.cargoTransferRotationEnabled,0);assert.equal(amr.parameters.cargoTransferAngle,0);assert.equal(forkKeys.includes('jerk'),false);assert.equal(amrKeys.includes('motionProfile'),false);});
...
test('메인 AS/RS 상자는 입고 셀 적재와 출고 인계 때만 점진적으로 소멸한다',()=>{const item={id:'asrs',type:'stackerCrane',x:0,y:0,parameters:{rows:1,columns:1,levels:1,infeedTime:1,putawayTime:2,retrievalTime:2,outfeedTime:2,forkStroke:.1,forkSpeed:10,forkAcceleration:10}},base={asrs:{equipmentId:'asrs',zones:{라인:{}}}},putaway={nodeId:'asrs',flowKey:'라인',asrsPhase:'putaway',nodeEnteredAt:0,asrsTarget:{index:0}},profileStart=asrsLayoutCargoPresentation(item,{...base,t:1.02},putaway),profileEnd=asrsLayoutCargoPresentation(item,{...base,t:3.2},putaway),retrieval={nodeId:'asrs',edge:{from:'asrs',to:'next'},flowKey:'라인',asrsPhase:'retrieval',nodeEnteredAt:0,asrsTarget:{index:0},handoffAcceptedAt:5},outStart=asrsLayoutCargoPresentation(item,{...base,t:5},retrieval),outEnd=asrsLayoutCargoPresentation(item,{...base,t:7},retrieval);assert.ok(profileStart.scale>profileEnd.scale);assert.equal(outStart.scale,1);assert.equal(outEnd.visible,false);});
