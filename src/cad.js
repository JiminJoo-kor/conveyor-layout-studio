...
export const equipmentParameterLabels = {
  ...
  travelSpeed:'주행 속도(m/s)',liftSpeed:'승강 속도(m/s)',loadCapacity:'적재 하중(kg)',lineSpeed:'라인 속도(m/min)',pitch:'차체 피치(m)',bufferCapacity:'라인 버퍼 수',forkTime:'포킹 동작시간(초)',strokeDistance:'포크 스트로크(m)'
...
export function buildSchematicLayout(candidates){
  ...
  const forkingDevices=nodes.filter(item=>item.type==='forkingDevice'),flowNodes=nodes.filter(item=>item.type!=='processLine'&&!storage.includes(item)&&!bridges.includes(item)&&!forkingDevices.includes(item)),lanes=[];
  ...
  const usedForks=new Set();for(let index=0;index<edges.length;index++){const edge=edges[index];if(edge.kind!=='warehouse'||!forkingDevices.length)continue;const other=nodes.find(item=>item.id===(edge.from===warehouse?.id?edge.to:edge.from)),fork=[...forkingDevices].filter(item=>!usedForks.has(item.id)).sort((a,b)=>Math.abs(a.y-(other?.y||warehouse.y))-Math.abs(b.y-(other?.y||warehouse.y)))[0];if(!fork)continue;usedForks.add(fork.id);edges.splice(index,1,...(edge.to===warehouse.id?[{from:edge.from,to:fork.id,kind:'forking'},{from:fork.id,to:warehouse.id,kind:'warehouse'}]:[{from:warehouse.id,to:fork.id,kind:'warehouse'},{from:fork.id,to:edge.to,kind:'forking'}]));index++;}
  return {lanes:labels.length>=2?lanes:lanes.filter(lane=>lane.nodes.length>1),edges,warehouseId:warehouse?.id,inferredEquipment,usedForkingDeviceIds:[...usedForks]};
}
...
export function normalizeSchematicPositions(candidates,schematic,width=1200){
  if(schematic.lanes.length>=3&&schematic.lanes.every(lane=>lane.labelId)){
    const positionById=new Map(),laneYs=[125,245,475],warehouseX=width-130,warehouseY=300;
    ...
    for(const bridge of candidates.filter(item=>['agv','amr'].includes(item.type)))positionById.set(bridge.id,{x:900,y:laneYFor(bridge)});
    for(const fork of candidates.filter(item=>item.type==='forkingDevice')){const connectedBridge=candidates.find(item=>['agv','amr'].includes(item.type)&&schematic.edges.some(edge=>(edge.from===fork.id&&edge.to===item.id)||(edge.to===fork.id&&edge.from===item.id))),bridgePosition=positionById.get(connectedBridge?.id);positionById.set(fork.id,{x:995,y:bridgePosition?.y??laneYFor(fork)});}
...
export async function analyzeCadFile(file) {
  ...
  const schematic=buildSchematicLayout(candidates);candidates=[...candidates,...(schematic.inferredEquipment||[])].filter(item=>item.type!=='forkingDevice'||schematic.usedForkingDeviceIds?.includes(item.id));candidates=normalizeSchematicPositions(candidates,schematic,1200);schematic.lanes.forEach(lane=>lane.nodes=lane.nodes.map(node=>candidates.find(item=>item.id===node.id)||node));
