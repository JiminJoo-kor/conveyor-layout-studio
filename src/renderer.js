...
import { itemVisualLength, smoothedVelocityProgress } from './occupancy.js';
...
export function stableCargoVisualMetrics(layout,cargo,commonVisualLength=78){const lengths=(layout?.equipment||[]).filter(item=>item.type==='conveyor'&&item.source?.origin==='dxf').map(item=>Math.max(.1,Number(item.parameters?.length)||cargo.length)).sort((a,b)=>a-b),referenceLength=lengths.length?lengths[Math.floor(lengths.length/2)]:Math.max(cargo.length,5),scale=commonVisualLength/referenceLength;return{scale,visualLength:Math.max(6,Math.min(commonVisualLength*.9,cargo.length*scale)),visualWidth:Math.max(5,Math.min(18,cargo.width*scale))};}
export function conveyorCargoVisualPose(item,cargo,physicalPosition,commonVisualLength=78,fixedMetrics=null){const physicalLength=Math.max(.1,Number(item?.parameters?.length)||cargo.length),scale=commonVisualLength/physicalLength,angle=(Number(item?.rotation)||0)*Math.PI/180,local=-commonVisualLength/2+(Math.max(0,Number(physicalPosition)||0)-cargo.length/2)*scale;return{x:item.x+Math.cos(angle)*local,y:item.y+Math.sin(angle)*local,angle,visualLength:fixedMetrics?.visualLength??Math.max(1,cargo.length*scale),visualWidth:fixedMetrics?.visualWidth??Math.max(5,Math.min(24,cargo.width*24/physicalLength)),scale};}
...
export class LayoutRenderer {
  ...
  handoverDescriptor(token,state,nodes,cargo){const physical=item=>Math.max(.1,Number(item.parameters?.length)||cargo.length);if(!token.edge&&token.motion&&['conveyor','processLine','sorter'].includes(nodes.get(token.nodeId)?.type)){const source=nodes.get(token.nodeId),position=Number(token.motionState?.position)||0,entered=Math.max(0,position-physical(source)),outgoing=(this.layout.cadSchematic?.edges||[]).filter(edge=>edge.from===source.id),edge=token.visualEdge||outgoing[0];if(entered>0&&entered<cargo.length&&edge)return{source,target:nodes.get(edge.to),edge,raw:entered/cargo.length};}const handover=token.handover,source=nodes.get(handover?.sourceId),target=nodes.get(handover?.targetId);if(!handover||!source||!target)return null;const targetPosition=token.nodeId===target.id?Number(token.motionState?.position):NaN,raw=Number.isFinite(targetPosition)?targetPosition/cargo.length:(state.t-handover.startedAt)/Math.max(.001,handover.duration);return raw<1?{source,target,edge:handover.edge||token.edge||{},raw:Math.max(0,raw)}:null;}

  drawHandoverOverlay(state){const c=this.ctx,nodes=new Map(this.layout.equipment.map(item=>[item.id,item])),cargo=normalizedCargoSpec(this.layout),commonVisualLength=this.layout.cadViewMode==='hybrid'?58:78,metrics=stableCargoVisualMetrics(this.layout,cargo,commonVisualLength),visualSize=item=>item.type==='conveyor'?commonVisualLength:44,physicalSize=item=>Math.max(.1,Number(item.parameters?.length)||cargo.length),speed=item=>Math.max(.05,Number(item.parameters?.speed)||Number(item.parameters?.travelSpeed)||1);for(const token of state.cadTokens||[]){const descriptor=this.handoverDescriptor(token,state,nodes,cargo);if(!descriptor?.source||!descriptor?.target)continue;const {source,target,edge,raw}=descriptor,sourceScale=visualSize(source)/physicalSize(source),targetScale=visualSize(target)/physicalSize(target),sourceVelocity=speed(source)*sourceScale,targetVelocity=speed(target)*targetScale,routeProgress=smoothedVelocityProgress(raw,sourceVelocity,targetVelocity),sourcePose=conveyorCargoVisualPose(source,cargo,physicalSize(source),visualSize(source),metrics),targetPose=conveyorCargoVisualPose(target,cargo,cargo.length,visualSize(target),metrics),route=edgeRoute({x:sourcePose.x,y:sourcePose.y},{x:targetPose.x,y:targetPose.y},edge),point=pointOnRoute(route,routeProgress),before=pointOnRoute(route,Math.max(0,routeProgress-.005)),after=pointOnRoute(route,Math.min(1,routeProgress+.005)),angle=Math.atan2(after.y-before.y,after.x-before.x),height=metrics.visualWidth,color=flowColor(token.flowKey,token.flowIndex);c.save();c.beginPath();for(let index=1;index<route.length;index++){const a=route[index-1],b=route[index],padding=Math.max(metrics.visualLength/2+2,height);c.rect(Math.min(a.x,b.x)-padding,Math.min(a.y,b.y)-padding,Math.abs(b.x-a.x)+padding*2,Math.abs(b.y-a.y)+padding*2);}c.clip();c.translate(point.x,point.y);c.rotate(angle);c.fillStyle=color;c.strokeStyle='rgba(255,255,255,.85)';c.lineWidth=1;c.fillRect(-metrics.visualLength/2,-height/2,metrics.visualLength,height);c.strokeRect(-metrics.visualLength/2,-height/2,metrics.visualLength,height);c.restore();}}

  drawCadFlow(state){
    const c=this.ctx,conveyors=this.layout.equipment.filter(item=>item.type==='conveyor'&&item.source?.origin==='dxf');
    if(state.cadTokens){
      const nodes=new Map(this.layout.equipment.filter(item=>item.source?.origin==='dxf').map(item=>[item.id,item])),cargo=normalizedCargoSpec(this.layout),metrics=stableCargoVisualMetrics(this.layout,cargo,this.layout.cadViewMode==='hybrid'?58:78);
      for(const token of state.cadTokens){
        if(this.handoverDescriptor(token,state,nodes,cargo))continue;
        ...
        else if(!token.edge&&fromNode.type==='conveyor'){const visualLength=this.layout.cadViewMode==='hybrid'?58:78,physicalPosition=Number(token.motionState?.position)||0,pose=conveyorCargoVisualPose(fromNode,cargo,physicalPosition,visualLength,metrics),color=flowColor(token.flowKey,token.flowIndex);c.save();c.translate(fromNode.x,fromNode.y);c.rotate(pose.angle);c.beginPath();c.rect(-visualLength/2,-14,visualLength,28);c.clip();c.rotate(-pose.angle);c.translate(-fromNode.x,-fromNode.y);c.translate(pose.x,pose.y);c.rotate(pose.angle);c.fillStyle=color;c.strokeStyle='rgba(255,255,255,.85)';c.fillRect(-pose.visualLength/2,-pose.visualWidth/2,pose.visualLength,pose.visualWidth);c.strokeRect(-pose.visualLength/2,-pose.visualWidth/2,pose.visualLength,pose.visualWidth);c.restore();continue;}
        else{const from=connectionAnchor(fromNode,token.edge?.fromPort),to=connectionAnchor(toNode,token.edge?.toPort);point=pointOnRoute(edgeRoute(from,to,token.edge||{}),token.edge?token.progress:0);}
        const x=point.x,y=point.y;c.fillStyle=flowColor(token.flowKey,token.flowIndex);c.strokeStyle='rgba(255,255,255,.85)';c.fillRect(x-cargoW/2,y-cargoH/2,cargoW,cargoH);c.strokeRect(x-cargoW/2,y-cargoH/2,cargoW,cargoH);
      }
      return;
    }
