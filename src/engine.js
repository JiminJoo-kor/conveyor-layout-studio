...
export const cargoSpec=layout=>({length:Math.max(.1,Number(layout?.cargoSpec?.length||1.2)),width:Math.max(.1,Number(layout?.cargoSpec?.width||.8))});
export const conveyorCargoCapacity=(item,layout)=>Math.max(1,Math.floor(equipmentLengthMeters(item,layout)/cargoSpec(layout).length));
export const cadDuration=(item,layout)=>{const p=item?.parameters||{};if(['conveyor','processLine'].includes(item?.type)){const length=equipmentLengthMeters(item,layout),metersPerSecond=equipmentSpeedMetersPerSecond(item);return Math.max(.2,length/Math.max(.05,metersPerSecond));}if(['asrs','stackerCrane'].includes(item?.type)){const travel=12/Math.max(.1,Number(p.travelSpeed||2.5)),lift=Math.max(1,Number(p.levels||1))*1.5/Math.max(.1,Number(p.liftSpeed||1));return Math.max(.2,travel+lift+4);}if(['agv','amr'].includes(item?.type)){const oneWay=Number(p.shuttleDistance||5)/Math.max(.1,Number(p.speed||1.2));return Math.max(.2,oneWay+Number(p.loadTime||2)+Number(p.unloadTime||2));}if(item?.type==='forklift')return Math.max(.2,Number(p.loadTime||8)+Number(p.unloadTime||8)+5/Math.max(.1,Number(p.speed||1.5)));if(item?.type==='turntable')return Math.max(.2,Number(p.rotationTime||6));if(item?.type==='handoffPoint')return Math.max(.2,Number(p.transferTime||2));if(item?.type==='forkingDevice')return Math.max(.2,Number(p.forkTime||4));return Math.max(.2,Number(p.cycleTime??p.processTime??p.dischargeTime??1));};
export class CadFlowEngine {
  ...
  reset(){const storage=[...this.nodes.values()].find(node=>['asrs','stackerCrane'].includes(node.type)),p=storage?.parameters||{},levels=Math.max(1,Math.round(Number(p.levels)||4)),rows=Math.max(1,Math.round(Number(p.rows)||2)),columns=Math.max(1,Math.round(Number(p.columns)||8)),productTypes=Math.max(1,Math.round(Number(p.productTypes)||3)),cellCount=levels*rows*columns,capacity=cellCount*productTypes,branchNames=(this.layout.cadSchematic?.inboundBranches||[]).map(branch=>branch.name),defaultNames=['트림 라인','화이날 라인','도어 라인'],names=Array.from({length:productTypes},(_,index)=>branchNames[index]||defaultNames[index]||`품목 ${index+1}`),zones=Object.fromEntries(names.map(name=>[name,{inventory:0,capacity:cellCount,putaways:0,retrievals:0}])),outboundTrucks=Object.fromEntries([...this.nodes.values()].filter(node=>node.type==='dock'&&node.parameters?.dockRole==='outbound').map(node=>[node.id,{loaded:0,capacity:Math.max(1,Number(node.parameters?.truckCapacity||8)),departures:0,lastDeparture:0}]));this.state={t:0,cadTokens:[],nextId:1,nextInjection:0,completedProducts:[],events:[],movedItems:0,routeAccumulators:{},locks:{},robot:{phase:'idle'},source:[],product:[],outboundTrucks,asrs:{equipmentId:storage?.id,inventory:0,capacity,cellCount,levels,rows,columns,productTypes,zones,putaways:0,retrievals:0,busyTime:0}};return this.state;}
  emit(type,detail={}){this.state.events.push({t:this.state.t,type,...detail});}
  selectRoute(options,current,token){if(options.length<2)return 0;let fork=current?.type==='forkingDevice'?current:null,forkIndex=-1,primaryIndex=0;if(!fork){forkIndex=options.findIndex(edge=>this.nodes.get(edge.to)?.type==='forkingDevice');if(forkIndex<0)return token.flowIndex%options.length;fork=this.nodes.get(options[forkIndex].to);primaryIndex=options.findIndex((edge,index)=>index!==forkIndex&&this.nodes.get(edge.to)?.type!=='forkingDevice');if(primaryIndex<0)primaryIndex=options.findIndex((edge,index)=>index!==forkIndex);}else forkIndex=options.length>1?1:0;const ratio=Math.max(0,Math.min(100,Number(fork.parameters?.output1Ratio??50))),key=fork.id,previous=this.state.routeAccumulators[key],accumulator=(previous===undefined?100-ratio:previous)+ratio,choosePrimary=accumulator>=100;this.state.routeAccumulators[key]=choosePrimary?accumulator-100:accumulator;const optionIndex=choosePrimary?primaryIndex:forkIndex;this.emit('fork-routed',{equipmentId:fork.id,junctionId:current?.id,output:optionIndex+1,route:choosePrimary?'primary':'fork',ratio,sequence:'deterministic'});return optionIndex;}
  preselectEntryFork(token,toNode){if(toNode?.type!=='conveyor')return null;const options=this.outgoing.get(toNode.id)||[],forkIndex=options.findIndex(edge=>this.nodes.get(edge.to)?.type==='forkingDevice');if(options.length<2||forkIndex<0)return null;const optionIndex=this.selectRoute(options,toNode,token);return{options,optionIndex,forkIndex};}
  step(dt){const s=this.state;if(!(dt>0)||s.t>=this.params.simDuration)return s;s.t+=Math.min(dt,this.params.simDuration-s.t);if(s.t>=s.nextInjection){for(const [flowIndex,source] of this.sources.entries()){const branch=(this.layout.cadSchematic?.inboundBranches||[]).find(item=>item.nodeIds?.includes(source.id)),operationDuration=cadDuration(source,this.layout);s.cadTokens.push({id:s.nextId++,nodeId:source.id,createdAt:s.t,nodeEnteredAt:s.t,operationDuration,readyAt:s.t+operationDuration,edge:null,progress:0,flowIndex,flowKey:branch?.name||Object.keys(s.asrs.zones)[flowIndex%Math.max(1,Object.keys(s.asrs.zones).length)]});}s.nextInjection=s.t+Math.max(1,this.params.injectA||30);this.emit('source-injected',{equipmentId:this.sources.map(x=>x.id).join(',')});}
    for(const token of [...s.cadTokens]){if(token.nodeId===s.asrs.equipmentId&&!token.edge)s.asrs.busyTime+=dt;if(token.edge){const toNode=this.nodes.get(token.edge.to),zone=s.asrs.zones[token.flowKey],exclusive=['agv','amr','turntable'].includes(toNode.type),capacity=toNode.type==='conveyor'?conveyorCargoCapacity(toNode,this.layout):Infinity,occupancy=s.cadTokens.filter(other=>other!==token&&!other.edge&&other.nodeId===toNode.id).length;if((exclusive&&occupancy>0)||occupancy>=capacity)continue;if(toNode.id===s.asrs.equipmentId&&zone&&zone.inventory>=zone.capacity)continue;const entryFork=this.preselectEntryFork(token,toNode);if(entryFork&&entryFork.optionIndex===entryFork.forkIndex){token.nodeId=toNode.id;token.edge=entryFork.options[entryFork.optionIndex];token.progress=1;s.movedItems++;continue;}if(entryFork)token.preselectedRoute={nodeId:toNode.id,optionIndex:entryFork.optionIndex};const operationDuration=cadDuration(toNode,this.layout);token.nodeId=toNode.id;token.edge=null;token.progress=0;token.nodeEnteredAt=s.t;token.operationDuration=operationDuration;token.readyAt=s.t+operationDuration;s.movedItems++;if(toNode.id===s.asrs.equipmentId){s.asrs.inventory=Math.min(s.asrs.capacity,s.asrs.inventory+1);s.asrs.putaways++;if(zone){zone.inventory++;zone.putaways++;}this.emit('asrs-putaway',{equipmentId:toNode.id,zone:token.flowKey});}this.emit('equipment-start',{equipmentId:toNode.id});}else if(s.t>=token.readyAt){const options=this.outgoing.get(token.nodeId)||[];if(options.length){if(token.nodeId===s.asrs.equipmentId){const zone=s.asrs.zones[token.flowKey];s.asrs.inventory=Math.max(0,s.asrs.inventory-1);s.asrs.retrievals++;if(zone){zone.inventory=Math.max(0,zone.inventory-1);zone.retrievals++;}this.emit('asrs-retrieval',{equipmentId:token.nodeId,zone:token.flowKey});}const current=this.nodes.get(token.nodeId),preselected=token.preselectedRoute?.nodeId===token.nodeId?token.preselectedRoute:null,optionIndex=preselected?preselected.optionIndex:this.selectRoute(options,current,token);token.preselectedRoute=null;token.edge=options[optionIndex];token.progress=1;}else{const truck=s.outboundTrucks?.[token.nodeId];if(truck){truck.loaded++;if(truck.loaded>=truck.capacity){truck.departures++;truck.loaded=0;truck.lastDeparture=s.t;this.emit('truck-departed',{equipmentId:token.nodeId,departures:truck.departures});}}s.completedProducts.push({id:token.id,cycleTime:s.t-token.createdAt});s.cadTokens.splice(s.cadTokens.indexOf(token),1);this.emit('equipment-complete',{equipmentId:token.nodeId,productId:token.id});}}}return s;}
