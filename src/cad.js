...
export function normalizeSchematicPositions(candidates,schematic,width=1200){
  if(schematic.lanes.length>=3&&schematic.lanes.every(lane=>lane.labelId)){
    ...
    schematic.lanes.forEach((lane,laneIndex)=>{const y=laneYs[laneIndex]??125+laneIndex*120,count=lane.nodes.length,gap=count>1?Math.min(145,680/(count-1)):0;lane.nodes.forEach((node,index)=>{const x=lane.direction==='inbound'?95+index*gap:820-index*gap;positionById.set(node.id,{x,y});});const label=candidates.find(item=>item.id===lane.labelId);if(label)positionById.set(label.id,{x:70,y:y-28});});
    ...
    for(const fork of candidates.filter(item=>item.type==='forkingDevice')){const forkHandoffs=candidates.filter(item=>item.type==='handoffPoint'&&adjacentIds(fork.id).includes(item.id)),connectedBridge=candidates.find(item=>['agv','amr'].includes(item.type)&&forkHandoffs.some(hp=>adjacentIds(hp.id).includes(item.id))),bridgePosition=positionById.get(connectedBridge?.id);positionById.set(fork.id,{x:980,y:bridgePosition?.y??laneYFor(fork)});}
    for(const hp of candidates.filter(item=>item.type==='handoffPoint')){const connectedFork=candidates.find(item=>item.type==='forkingDevice'&&adjacentIds(hp.id).includes(item.id)),forkPosition=positionById.get(connectedFork?.id),warehouseSide=adjacentIds(hp.id).includes(schematic.warehouseId);positionById.set(hp.id,{x:warehouseSide?1040:925,y:forkPosition?.y??laneYFor(hp)});}
