...
export const equipmentLengthMeters=(item,layout)=>{const configured=Number(item?.parameters?.length),scale=unitScale(layout?.cadSource?.units);if(Number.isFinite(configured)&&configured>0)return item?.source?.origin==='dxf'&&scale&&item?.source?.parameterLengthUnit!=='m'?configured*scale:configured;return Math.max(.1,Number(item?.length||item?.width||20)/20);};
...
export class CadFlowEngine {
  ...
  reset(){const storage=[...this.nodes.values()].find(node=>['asrs','stackerCrane'].includes(node.type)),p=storage?.parameters||{},levels=Math.max(1,Math.round(Number(p.levels)||4)),rows=Math.max(1,Math.round(Number(p.rows)||3)),columns=Math.max(1,Math.round(Number(p.columns)||4)),productTypes=Math.max(1,Math.round(Number(p.productTypes)||3)),capacity=levels*rows*columns,branchNames=(this.layout.cadSchematic?.inboundBranches||[]).map(branch=>branch.name),defaultNames=['트림 라인','화이날 라인','도어 라인'],names=Array.from({length:productTypes},(_,index)=>branchNames[index]||defaultNames[index]||`품목 ${index+1}`),baseCapacity=Math.floor(capacity/productTypes),remainder=capacity%productTypes,zones=Object.fromEntries(names.map((name,index)=>[name,{inventory:0,capacity:baseCapacity+(index<remainder?1:0),putaways:0,retrievals:0}])),outboundTrucks=Object.fromEntries([...this.nodes.values()].filter(node=>node.type==='dock'&&node.parameters?.dockRole==='outbound').map(node=>[node.id,{loaded:0,capacity:Math.max(1,Number(node.parameters?.truckCapacity||8)),departures:0,lastDeparture:0}]));this.state={t:0,cadTokens:[],nextId:1,nextInjection:0,completedProducts:[],events:[],movedItems:0,routeAccumulators:{},locks:{},robot:{phase:'idle'},source:[],product:[],outboundTrucks,asrs:{equipmentId:storage?.id,inventory:0,capacity,levels,rows,columns,productTypes,zones,putaways:0,retrievals:0,busyTime:0}};return this.state;}
