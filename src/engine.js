...
export const cargoSpec=layout=>{const spec=layout?.cargoSpec||{},millimeters=spec.unit==='mm'||Number(spec.length)>20||Number(spec.width)>20,scale=millimeters ? .001 : 1;return{length:Math.max(.001,Number(spec.length||(millimeters?1200:1.2))*scale),width:Math.max(.001,Number(spec.width||(millimeters?800:.8))*scale),unit:'m'};};
...
export const asrsCycleDuration=(item,{slotIndex=0,operation='putaway'}={})=>{const p=item?.parameters||{},target=asrsTargetCell(item,slotIndex),horizontal=(target.column+1)*Math.max(.1,Number(p.columnPitch)||1.5),vertical=(target.level+1)*Math.max(.1,Number(p.levelHeight)||1.5),travel=acceleratedTravelTime(horizontal,p.travelSpeed||2.5,p.acceleration||.5),lift=acceleratedTravelTime(vertical,operation==='retrieval'?(p.downSpeed||p.liftSpeed||1):(p.liftSpeed||1),p.acceleration||.5),motion=Number(p.simultaneousMotion??1)?Math.max(travel,lift):travel+lift,fork=Math.max(0,Number(operation==='retrieval'?p.retrievalTime:p.putawayTime)||4),returnTime=Number(p.returnHome||0)?motion:0;return Math.max(.2,motion+fork+returnTime);};
export const cadDuration=(item,layout,context={})=>{const p=item?.parameters||{};if(['conveyor','processLine','sorter'].includes(item?.type)){const length=equipmentLengthMeters(item,layout),cargoLength=cargoSpec(layout).length,metersPerSecond=equipmentSpeedMetersPerSecond(item);return Math.max(.2,(length+cargoLength)/Math.max(.05,metersPerSecond));}if(['asrs','stackerCrane'].includes(item?.type))return asrsCycleDuration(item,context);if(['agv','amr','shuttle'].includes(item?.type)){const oneWay=Number(p.shuttleDistance||5)/Math.max(.1,Number(p.speed||1.2));return Math.max(.2,oneWay+Number(p.loadTime||2)+Number(p.unloadTime||2));}if(item?.type==='forklift')return Math.max(.2,Number(p.loadTime||8)+Number(p.unloadTime||8)+Number(p.travelDistance||5)/Math.max(.1,Number(p.speed||1.5)));if(item?.type==='lift')return Math.max(.2,Number(p.loadTime||2)+Number(p.unloadTime||2)+Number(p.liftHeight||3)/Math.max(.1,Number(p.liftSpeed||.5)));if(item?.type==='turntable')return Math.max(.2,Number(p.rotationTime||6));if(item?.type==='handoffPoint')return Math.max(.2,Number(p.transferTime||2));if(item?.type==='forkingDevice')return Math.max(.2,Number(p.forkTime||4));return Math.max(.2,Number(p.cycleTime??p.processTime??p.dischargeTime??1));};
