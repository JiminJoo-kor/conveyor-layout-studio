...
export function buildSimulationReport(layout,engine,targetUph=0){
  ...
  return{corrections,topology:(layout.cadSchematic?.lanes||[]).map(lane=>`${lane.name}: ${lane.direction==='inbound'?'입고 → AGV/AMR → AS/RS 보관':'AS/RS 반출 → AGV → 공정/출고'}`),rows,kpis,targetUph:target,realizableUph:realizable,eightHours:realizable*8,twelveHours:realizable*12};
}
