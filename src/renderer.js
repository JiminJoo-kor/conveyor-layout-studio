...
import { connectionAnchor, equipmentPorts, orthogonalRoute, pointOnRoute, routeArrow, routeLength } from './route.js';
...
export class LayoutRenderer {
  ...
  drawCadSymbol(item,state){
    ...
    if(item.type==='conveyor'){const w=compact?58:78,h=compact?16:24,p=item.parameters||{},cargo=this.layout.cargoSpec||{length:1.2,width:.8};c.save();c.translate(item.x,item.y);c.fillStyle='#102b3b';c.strokeStyle=this.isSelected(item.id)?COLORS.yellow:COLORS.cyan;c.lineWidth=this.isSelected(item.id)?2:1.5;c.fillRect(-w/2,-h/2,w,h);c.strokeRect(-w/2,-h/2,w,h);for(let roller=-w/2+9;roller<w/2;roller+=13){c.beginPath();c.arc(roller,0,3,0,Math.PI*2);c.stroke();}c.fillStyle='#d8f3ff';c.font=`${compact?6:8}px monospace`;c.textAlign='center';c.fillText('CONVEYOR',0,compact?-12:-17);c.fillStyle='#9fc5dd';c.fillText(`CV ${Number(p.length||5).toFixed(1)}m · 물류 ${Number(cargo.length||1.2).toFixed(1)}×${Number(cargo.width||.8).toFixed(1)}m`,0,compact?22:28);c.textAlign='start';c.restore();return true;}
    ...
    if(['amr','agv'].includes(item.type)){const half=compact?11:18,route=mobileEquipmentRoute(this.layout,item),operation=equipmentOperationProgress(item,state);let x=route?.start?.x??item.x,y=route?.start?.y??item.y;if(route){const points=route.points||[route.start,route.end],point=pointOnRoute(points,operation.progress);x=point.x;y=point.y;}c.save();c.translate(x,y);c.fillStyle=operation.active?'#26506a':'#15374a';c.strokeStyle=COLORS.yellow;c.fillRect(-half,-half*.7,half*2,half*1.4);c.strokeRect(-half,-half*.7,half*2,half*1.4);c.fillStyle='#fff';c.font=`${compact?7:9}px monospace`;c.fillText(item.type.toUpperCase(),compact?-7:-10,3);c.restore();return true;}
    return false;
  }

  drawCadFlow(state){
    const c=this.ctx,conveyors=this.layout.equipment.filter(item=>item.type==='conveyor'&&item.source?.origin==='dxf');
    if(state.cadTokens){const nodes=new Map(this.layout.equipment.filter(item=>item.source?.origin==='dxf').map(item=>[item.id,item])),cargo=this.layout.cargoSpec||{length:1.2,width:.8};for(const token of state.cadTokens){const fromNode=nodes.get(token.edge?.from||token.nodeId),toNode=nodes.get(token.edge?.to||token.nodeId);if(!fromNode||!toNode)continue;let point,cargoW=14,cargoH=14;if(!token.edge&&['agv','amr'].includes(fromNode.type)){const route=mobileEquipmentRoute(this.layout,fromNode),operation=equipmentOperationProgress(fromNode,state);point=route?pointOnRoute(route.points||[route.start,route.end],operation.progress):{x:fromNode.x,y:fromNode.y};}else if(!token.edge&&fromNode.type==='conveyor'){const operation=equipmentOperationProgress(fromNode,state),visualLength=this.layout.cadViewMode==='hybrid'?58:78,angle=(fromNode.rotation||0)*Math.PI/180,p=fromNode.parameters||{},physicalLength=Math.max(.1,Number(p.length||5));cargoW=Math.max(8,Math.min(26,visualLength*Number(cargo.length||1.2)/physicalLength));cargoH=Math.max(7,Math.min(18,8+Number(cargo.width||.8)*5));const offset=-visualLength/2+cargoW/2+operation.progress*Math.max(0,visualLength-cargoW);point={x:fromNode.x+Math.cos(angle)*offset,y:fromNode.y+Math.sin(angle)*offset};}else{const from=connectionAnchor(fromNode,token.edge?.fromPort),to=connectionAnchor(toNode,token.edge?.toPort);point=pointOnRoute(orthogonalRoute(from,to),token.edge?token.progress:0);}const x=point.x,y=point.y;c.fillStyle=COLORS.yellow;c.strokeStyle='#fff2a8';c.fillRect(x-cargoW/2,y-cargoH/2,cargoW,cargoH);c.strokeRect(x-cargoW/2,y-cargoH/2,cargoW,cargoH);}return;}
