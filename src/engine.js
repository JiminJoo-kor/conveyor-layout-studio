...
export const cadDuration=(item,layout,context={})=>{
  ...
  if(['conveyor','processLine','sorter'].includes(item?.type)){const length=equipmentLengthMeters(item,layout),cargoLength=cargoSpec(layout).length,metersPerSecond=equipmentSpeedMetersPerSecond(item);duration=(length+cargoLength)/Math.max(.05,metersPerSecond);}
