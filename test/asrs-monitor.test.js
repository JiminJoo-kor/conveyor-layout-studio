...
test('셀 적재 후반 물류는 목표 셀 중심에서 서서히 축소된다',()=>{const before=asrsDepthPresentation({status:'inbound',target:{row:1},phase:'fork',forkProgress:.5},2),middle=asrsDepthPresentation({status:'inbound',target:{row:1},phase:'fork',forkProgress:.75},2),done=asrsDepthPresentation({status:'inbound',target:{row:1},phase:'fork',forkProgress:1},2);assert.equal(before.cargoScale,1);assert.equal(middle.cargoScale,.5);assert.equal(done.cargoScale,0);});
