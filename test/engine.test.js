...
test('새 프로젝트의 임의 라인명도 시작 설비에서 물류 라인으로 고정된다',()=>{const sources=['차체 1라인','모듈 X라인'].map((name,index)=>({id:`src-${index}`,type:'source',source:{origin:'dxf'},parameters:{processTime:1},name})),branches=sources.map((source,index)=>({name:index?'모듈 X라인':'차체 1라인',nodeIds:[source.id]})),engine=new CadFlowEngine({equipment:sources,cadSchematic:{inboundBranches:branches,edges:[]}},{injectA:30,simDuration:2});engine.step(.02);assert.deepEqual(engine.state.cadTokens.map(token=>token.flowKey),['차체 1라인','모듈 X라인']);});
