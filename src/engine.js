...
export class CadFlowEngine {
  ...
  applyOccupancyInterlocks(dt){this.occupancy.clear();const cargo=cargoSpec(this.layout);for(const item of this.nodes.values()){if(!['conveyor','processLine','sorter'].includes(item.type))continue;const length=equipmentLengthMeters(item,this.layout),gap=Math.max(0,Number(item.parameters?.safetyGap)||0),tokens=this.state.cadTokens.filter(token=>token.nodeId===item.id).sort((a,b)=>((b.edge?1:b.progress)||0)-((a.edge?1:a.progress)||0));for(const token of tokens){const center=Math.max(cargo.length/2,Math.min(length-cargo.length/2,(token.edge?1:token.progress||0)*length));if(this.occupancy.canOccupy(token.id,item.id,center,cargo.length,gap)){this.occupancy.reserve(token.id,item.id,center,cargo.length,gap);if(token.queueState!=='ASRS출고대기')token.queueState=null;}else{token.readyAt+=dt;token.queueState='Queue';token.motionState={...(token.motionState||{}),state:'Stopping'};}}}}
