import { closestPortPair, connectionAnchor, connectionKind, edgeRoute } from './route.js';
...
export class LayoutEditor {
  constructor(canvas, renderer, getLayout, onChange, onSelect, onConnect, onModeChange, onEdgeSelect) {
    ...
    this.enabled=false;this.drag=null;this.edgeDrag=null;this.pan=null;this.marquee=null;this.selectedIds=new Set();this.connecting=false;this.connectionSource=null;this.placementType=null;this.view={zoom:1,x:0,y:0};
  ...
  pointerMove(event){if(this.placementType){this.renderer.setPlacementPreview(this.placementType,this.worldPoint(event));this.onChange(false);return;}if(!this.enabled&&!this.pan)return;if(this.pan){const p=this.canvasPoint(event);this.view.x=this.pan.start.x+p.x-this.pan.raw.x;this.view.y=this.pan.start.y+p.y-this.pan.raw.y;this.renderer.setView(this.view);this.onChange(false);return;}
    ...
    if(this.edgeDrag){const p=this.worldPoint(event),dx=Math.round((p.x-this.edgeDrag.start.x)/10)*10,dy=Math.round((p.y-this.edgeDrag.start.y)/10)*10;this.edgeDrag.edge.routeOffset={x:this.edgeDrag.offset.x+dx,y:this.edgeDrag.offset.y+dy};this.onChange(false);return;}
