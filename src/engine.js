...
export class CadFlowEngine {
  ...
  asrsInboundInterlocked(toNode){return toNode?.id===this.state.asrs.equipmentId&&this.state.cadTokens.some(token=>token.nodeId===toNode.id&&!token.edge&&token.asrsPhase==='retrieval');}
  holdAsrsInboundDuringRetrieval(){const storageId=this.state.asrs.equipmentId,busy=this.state.cadTokens.some(token=>token.nodeId===storageId&&!token.edge&&token.asrsPhase==='retrieval');if(!busy)return false;for(const token of this.state.cadTokens)if(token.edge?.to===storageId){token.edge=null;token.progress=1;token.blockedBy=storageId;token.queueState='ASRS출고대기';}return true;}
  scheduleAsrsBatchRetrieval(){const {storage,enabled,size}=this.asrsBatchConfig();this.holdAsrsInboundDuringRetrieval();if(!storage||!enabled)return false;for(const zone of Object.values(this.state.asrs.zones))if(zone.releaseRemaining<=0&&zone.inventory>=size)zone.releaseRemaining=Math.min(size,zone.inventory);if(this.state.cadTokens.some(token=>token.nodeId===storage.id&&['putaway','retrieval'].includes(token.asrsPhase)))return false;const token=this.state.cadTokens.find(entry=>entry.nodeId===storage.id&&entry.asrsPhase==='stored'&&(this.state.asrs.zones[entry.flowKey]?.releaseRemaining||0)>0);if(!token)return false;const duration=cadDuration(storage,this.layout,{slotIndex:token.asrsTarget?.index||0,operation:'retrieval'});token.asrsPhase='retrieval';token.nodeEnteredAt=this.state.t;token.operationDuration=duration;token.readyAt=this.state.t+duration;this.emit('asrs-retrieval-start',{equipmentId:storage.id,zone:token.flowKey,target:token.asrsTarget,duration,batch:true});this.holdAsrsInboundDuringRetrieval();return true;}
  ...
  resolveZeroDelayHandovers(){let changed=true,guard=this.nodes.size+1;while(changed&&guard-->0){changed=false;for(const token of this.state.cadTokens)if(token.edge&&!this.asrsInboundInterlocked(this.nodes.get(token.edge.to))&&this.tryDirectHandover(token))changed=true;}}
