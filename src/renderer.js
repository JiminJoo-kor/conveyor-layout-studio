  ...
  drawCadFlow(state){
    ...
    if(state.cadTokens){
      ...
      for(const token of state.cadTokens){
        if(token.handover&&state.t<token.handover.endAt)continue;
        ...
        else if(!token.edge&&fromNode.type==='conveyor'){const operation=equipmentOperationProgress(fromNode,state,token),visualLength=this.layout.cadViewMode==='hybrid'?58:78,angle=(fromNode.rotation||0)*Math.PI/180,p=fromNode.parameters||{},physicalLength=Math.max(.1,Number(p.length||5));cargoW=scaleRatioItemVisualSize(visualLength,cargo.length,physicalLength);cargoH=Math.max(5,Math.min(24,scaleRatioItemVisualSize(24,cargo.width,physicalLength)));const offset=-visualLength/2+cargoW/2+operation.progress*Math.max(0,visualLength-cargoW);point={x:fromNode.x+Math.cos(angle)*offset,y:fromNode.y+Math.sin(angle)*offset};}
