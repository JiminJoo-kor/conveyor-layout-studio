...
export class LayoutRenderer {
  ...
  drawHandoverOverlay(state){const c=this.ctx,nodes=new Map(this.layout.equipment.map(item=>[item.id,item])),cargo=normalizedCargoSpec(this.layout),commonVisualLength=this.layout.cadViewMode==='hybrid'?58:78,metrics=stableCargoVisualMetrics(this.layout,cargo,commonVisualLength),visualSize=item=>item.type==='conveyor'?commonVisualLength:item.type==='forkingDevice'?58:44,colorFor=token=>flowColor(token.flowKey,token.flowIndex),drawEndpoint=(item,fraction,isTarget,color)=>{if(!(fraction>0))return;const angle=(Number(item.rotation)||0)*Math.PI/180,size=visualSize(item),cargoLength=Math.min(metrics.visualLength,size*.9),cargoWidth=metrics.visualWidth,conveyor=item.type==='conveyor',center=conveyor?(isTarget?-size/2+cargoLength/2:size/2-cargoLength/2):0,visible=cargoLength*Math.min(1,fraction);c.save();c.translate(item.x,item.y);c.rotate(angle);c.beginPath();c.rect(center-cargoLength/2,-cargoWidth/2,visible,cargoWidth);c.clip();c.fillStyle=color;c.strokeStyle='rgba(255,255,255,.85)';c.lineWidth=1;c.fillRect(center-cargoLength/2,-cargoWidth/2,cargoLength,cargoWidth);c.strokeRect(center-cargoLength/2,-cargoWidth/2,cargoLength,cargoWidth);c.restore();};for(const token of state.cadTokens||[]){const descriptor=this.handoverDescriptor(token,state,nodes,cargo);if(!descriptor?.source||!descriptor?.target)continue;const raw=Math.max(0,Math.min(1,descriptor.raw)),color=colorFor(token);drawEndpoint(descriptor.source,1-raw,false,color);drawEndpoint(descriptor.target,raw,true,color);}}
