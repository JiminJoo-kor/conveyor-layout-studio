...
function renderRackMonitor(){const asrs=engine.state?.asrs,zones=Object.entries(asrs?.zones||{});if(!asrs?.equipmentId){rackMonitor.hidden=true;return;}rackMonitor.hidden=false;rackMonitor.innerHTML=`<h2>AS/RS 재고 모니터</h2><div class="rack-summary">총 재고 <strong>${asrs.inventory}/${asrs.capacity}</strong></div>${zones.map(([name,zone],zoneIndex)=>{const shown=Math.min(64,zone.capacity),filled=Math.min(shown,zone.inventory);return `<article class="rack-zone"><header><span>${name}</span><strong>${zone.inventory}/${zone.capacity}</strong></header><div class="rack-cells">${Array.from({length:shown},(_,index)=>`<i class="${index<filled?'occupied':''}" style="--rack-color:${flowColor(name,zoneIndex)}"></i>`).join('')}</div></article>`;}).join('')}`;}
...
function renderCadEquipmentParameters(selectedId=selectedEquipment?.id){
  ...
  if(active)appendForkParameterControls(items);
