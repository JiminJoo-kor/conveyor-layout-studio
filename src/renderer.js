...
import { handoverVisualSegments, itemVisualLength } from './occupancy.js';
...
export class LayoutRenderer {
  ...
  drawHandoverOverlay(state){const c=this.ctx,nodes=new Map(this.layout.equipment.map(item=>[item.id,item])),cargo=normalizedCargoSpec(this.layout);for(const token of state.cadTokens||[]){const handover=token.handover;if(!handover||state.t>=handover.endAt)continue;const source=nodes.get(handover.sourceId),target=nodes.get(handover.targetId);if(!source||!target)continue;const progress=Math.max(0,Math.min(1,(state.t-handover.startedAt)/Math.max(.001,handover.duration))),sourceVisual=itemVisualLength(source.type==='conveyor'?78:44,cargo.length,Math.max(.1,Number(source.parameters?.length)||cargo.length)),targetVisual=itemVisualLength(target.type==='conveyor'?78:44,cargo.length,Math.max(.1,Number(target.parameters?.length)||cargo.length)),sourceParts=handoverVisualSegments(progress,sourceVisual),targetParts=handoverVisualSegments(progress,targetVisual),sourceAnchor=connectionAnchor(source),targetAnchor=connectionAnchor(target),color=flowColor(token.flowKey,token.flowIndex);c.save();c.fillStyle=color;c.strokeStyle='rgba(255,255,255,.85)';const sourceAngle=(source.rotation||0)*Math.PI/180,targetAngle=(target.rotation||0)*Math.PI/180;if(sourceParts.sourceLength>0){c.translate(sourceAnchor.x,sourceAnchor.y);c.rotate(sourceAngle);c.fillRect(-sourceParts.sourceLength,-6,sourceParts.sourceLength,12);c.strokeRect(-sourceParts.sourceLength,-6,sourceParts.sourceLength,12);c.rotate(-sourceAngle);c.translate(-sourceAnchor.x,-sourceAnchor.y);}if(targetParts.targetLength>0){c.translate(targetAnchor.x,targetAnchor.y);c.rotate(targetAngle);c.fillRect(0,-6,targetParts.targetLength,12);c.strokeRect(0,-6,targetParts.targetLength,12);}c.restore();}}
