...
export class LayoutRenderer {
  ...
  drawCadSymbol(item,state){
    ...
    if(item.type==='conveyor'){const w=compact?58:78,h=compact?16:24,p=item.parameters||{};c.save();c.translate(item.x,item.y);c.fillStyle='#102b3b';c.strokeStyle=this.isSelected(item.id)?COLORS.yellow:COLORS.cyan;c.lineWidth=this.isSelected(item.id)?2:1.5;c.fillRect(-w/2,-h/2,w,h);c.strokeRect(-w/2,-h/2,w,h);for(let roller=-w/2+9;roller<w/2;roller+=13){c.beginPath();c.arc(roller,0,3,0,Math.PI*2);c.stroke();}c.fillStyle='#d8f3ff';c.font=`${compact?6:8}px monospace`;c.textAlign='center';c.fillText('CONVEYOR',0,compact?-12:-17);c.fillStyle='#9fc5dd';c.fillText(`CV ${Number(p.length||5).toFixed(1)}m · 물류 ${Number(p.cargoLength||1.2).toFixed(1)}×${Number(p.cargoWidth||.8).toFixed(1)}m`,0,compact?22:28);c.textAlign='start';c.restore();return true;}
