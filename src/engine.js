...
export class CadFlowEngine {
  ...
  asrsBatchConfig(){const storage=this.nodes.get(this.state.asrs.equipmentId),p=storage?.parameters||{};if(storage){storage.parameters??={};storage.parameters.stackerCount=this.state.asrs.productTypes;}return{storage,enabled:Number(p.batchReleaseEnabled||0)!==0,size:Math.max(1,Math.round(Number(p.releaseBatchSize)||1))};}
  ...
  asrsLineBusy(lineKey,exclude=null){return this.state.cadTokens.some(token=>token!==exclude&&token.nodeId===this.state.asrs.equipmentId&&!token.edge&&token.flowKey===lineKey&&['putaway','retrieval'].includes(token.asrsPhase));}
  asrsInboundInterlocked(toNode,tokenOrLine){const lineKey=typeof tokenOrLine==='string'?tokenOrLine:tokenOrLine?.flowKey;return toNode?.id===this.state.asrs.equipmentId&&this.asrsLineBusy(lineKey,typeof tokenOrLine==='object'?tokenOrLine:null);}
  ...
  scheduleAsrsBatchRetrieval(){const {storage,enabled,size}=this.asrsBatchConfig();this.holdAsrsInboundWhileBusy();if(!storage||!enabled)return false;for(const zone of Object.values(this.state.asrs.zones))if(zone.releaseRemaining<=0&&zone.inventory>=size)zone.releaseRemaining=Math.min(size,zone.inventory);const token=this.state.cadTokens.find(entry=>entry.nodeId===storage.id&&entry.asrsPhase==='stored'&&(this.state.asrs.zones[entry.flowKey]?.releaseRemaining||0)>0&&!this.asrsLineBusy(entry.flowKey,entry));if(!token)return false;const duration=cadDuration(storage,this.layout,{slotIndex:token.asrsTarget?.index||0,operation:'retrieval'});token.asrsPhase='retrieval';token.nodeEnteredAt=this.state.t;token.operationDuration=duration;token.readyAt=this.state.t+duration;this.emit('asrs-retrieval-start',{equipmentId:storage.id,zone:token.flowKey,target:token.asrsTarget,duration,batch:true});this.holdAsrsInboundWhileBusy();return true;}
