...
cadParameterSection.addEventListener('change',event=>{const input=event.target.closest('[data-equipment-id]');if(!input)return;const item=layout.equipment.find(entry=>entry.id===input.dataset.equipmentId);if(item){item.parameters??={};const integer=['levels','rows','columns','productTypes'].includes(input.dataset.parameter),value=Number(input.value);item.parameters[input.dataset.parameter]=integer?Math.max(1,Math.round(value||1)):value;if(['length','cargoLength','cargoWidth'].includes(input.dataset.parameter))item.parameters[input.dataset.parameter]=Math.max(.1,item.parameters[input.dataset.parameter]||.1);if(input.dataset.parameter==='length'){item.source??={};item.source.parameterLengthUnit='m';}input.value=item.parameters[input.dataset.parameter];resetEngine();}});
