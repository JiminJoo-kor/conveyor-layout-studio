...
export function equipmentPorts(item,distance=null){
  const storage=['asrs','stackerCrane'].includes(item?.type),horizontal=Number.isFinite(distance)?distance:item?.asrsStation?item.asrsStation.visualLength/2:storage?(item.asrsVisualBounds?.width||178)/2+8:44,vertical=Number.isFinite(distance)?distance:item?.asrsStation?item.asrsStation.visualWidth/2:storage?(item.asrsVisualBounds?.height||132)/2+8:44,angle=(Number(item?.rotation)||0)*Math.PI/180,rotate=(x,y)=>({x:item.x+x*Math.cos(angle)-y*Math.sin(angle),y:item.y+x*Math.sin(angle)+y*Math.cos(angle)}),ports={left:rotate(-horizontal,0),right:rotate(horizontal,0),top:rotate(0,-vertical),bottom:rotate(0,vertical)};
  if(storage){const count=Math.max(1,Math.round(Number(item.parameters?.productTypes)||1)),span=Number(item.parameters?.stationConveyorsEnabled)===1?Math.max(0,(count-1)*(Number(item.parameters?.stationVisualSpacing)||60)*(item.parameters?.infeedSide===item.parameters?.outfeedSide&&item.parameters?.infeedSide?2:1)):Math.min(104,Math.max(0,(count-1)*26)),start=-span/2,infeedSide=['left','right','top','bottom'].includes(item.parameters?.infeedSide)?item.parameters.infeedSide:'left',outfeedSide=['left','right','top','bottom'].includes(item.parameters?.outfeedSide)?item.parameters.outfeedSide:'right',sameSide=infeedSide===outfeedSide,point=(side,index,kind)=>{const lane=count===1?0:start+span*index/(count-1)+(sameSide?(kind==='in'?-1:1)*(Number(item.parameters?.stationConveyorsEnabled)===1?(Number(item.parameters?.stationVisualSpacing)||60)/2:8):0);if(side==='right')return rotate(horizontal,lane);if(side==='top')return rotate(lane,-vertical);if(side==='bottom')return rotate(lane,vertical);return rotate(-horizontal,lane);};for(let index=0;index<count;index++){ports[`product-${index+1}-in`]=point(infeedSide,index,'in');ports[`product-${index+1}-out`]=point(outfeedSide,index,'out');}}
  return ports;
}

export function connectableEquipmentPorts(item){const ports=equipmentPorts(item);if(['asrs','stackerCrane'].includes(item?.type)&&Number(item.parameters?.stationConveyorsEnabled)===1)return {};if(item?.asrsStation){const side=item.asrsStation.kind==='in'?'left':'right';return{[side]:ports[side]};}return ports;}
