...
export class LayoutRenderer {
  ...
  drawCadSchematic(){
    ...
    for(const edge of schematic.edges||[]){if(this.layout.cadViewMode==='hybrid'&&edge.kind==='flow')continue;const from=nodeMap.get(edge.from),to=nodeMap.get(edge.to);if(!from||!to)continue;const transfer=edge.kind==='transfer',warehouse=edge.kind==='warehouse',distance=Math.hypot(to.x-from.x,to.y-from.y);if(this.layout.cadViewMode==='hybrid'&&distance>260)continue;c.strokeStyle=transfer?'rgba(255,113,57,.75)':warehouse?'rgba(0,255,136,.58)':'rgba(0,212,255,.48)';c.lineWidth=transfer?3:5;c.setLineDash(transfer?[7,5]:[]);c.beginPath();c.moveTo(from.x,from.y);c.lineTo(to.x,to.y);c.stroke();const angle=Math.atan2(to.y-from.y,to.x-from.x),x=from.x+(to.x-from.x)*.68,y=from.y+(to.y-from.y)*.68;c.setLineDash([]);c.fillStyle=transfer?COLORS.orange:warehouse?COLORS.green:COLORS.cyan;c.beginPath();c.moveTo(x+Math.cos(angle)*9,y+Math.sin(angle)*9);c.lineTo(x+Math.cos(angle+2.55)*8,y+Math.sin(angle+2.55)*8);c.lineTo(x+Math.cos(angle-2.55)*8,y+Math.sin(angle-2.55)*8);c.closePath();c.fill();}
  ...
  drawCadSymbol(item){
    ...
    if(['amr','agv'].includes(item.type)){const half=compact?11:22;c.save();c.translate(item.x,item.y);c.rotate(Math.PI/4);c.fillStyle='#15374a';c.strokeStyle=COLORS.yellow;c.fillRect(-half,-half,half*2,half*2);c.strokeRect(-half,-half,half*2,half*2);c.rotate(-Math.PI/4);c.fillStyle='#fff';c.font=`${compact?7:9}px monospace`;c.fillText(item.type.toUpperCase(),compact?-7:-10,3);c.restore();return true;}
    return false;
  }
