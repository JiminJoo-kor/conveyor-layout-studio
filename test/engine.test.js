...
test('포킹으로 다른 라인에 진입한 물류는 원래 품목색을 유지하고 도착 AS/RS 라인에 적재된다',()=>{const source={id:'src-a',type:'source',source:{origin:'dxf'}},infeed={id:'in-b',type:'conveyor',source:{origin:'dxf',parameterLengthUnit:'m'},parameters:{length:2,speed:1}},asrs={id:'asrs',type:'stackerCrane',source:{origin:'dxf'},parameters:{rows:1,columns:2,levels:1,productTypes:2}},edge={from:'in-b',to:'asrs'},branches=[{name:'트림 라인',nodeIds:['src-a']},{name:'화이날 라인',nodeIds:['in-b']}],engine=new CadFlowEngine({equipment:[source,infeed,asrs],cadSchematic:{inboundBranches:branches,edges:[edge]}},{injectA:99,simDuration:10}),token=engine.prepareToken({id:1,nodeId:'in-b',edge,flowKey:'트림 라인',cargoType:'트림 물류',flowIndex:0,progress:1,readyAt:0,motion:null});engine.state.cadTokens=[token];engine.resolveZeroDelayHandovers();assert.equal(token.originFlowKey,'트림 라인');assert.equal(token.cargoType,'트림 물류');assert.equal(token.flowKey,'화이날 라인');assert.equal(engine.state.asrs.zones['트림 라인'].inventory,0);assert.equal(engine.state.asrs.zones['화이날 라인'].inventory,1);});
...
test('서로 역방향인 설비 연결에서 반대편 인계가 진행 중이면 충돌 방지를 위해 대기한다',()=>{const a={id:'a',type:'forkingDevice',source:{origin:'dxf'},parameters:{}},b={id:'b',type:'forkingDevice',source:{origin:'dxf'},parameters:{}},ab={from:'a',to:'b'},ba={from:'b',to:'a'},engine=new CadFlowEngine({equipment:[a,b],cadSchematic:{edges:[ab,ba]}},{simDuration:10}),forward=engine.prepareToken({id:1,nodeId:'a',edge:ab}),reverse=engine.prepareToken({id:2,nodeId:'b',edge:ba});engine.state.cadTokens=[forward,reverse];assert.equal(engine.hasOpposingTransfer(ab,forward),true);assert.equal(engine.tryDirectHandover(forward),false);assert.equal(forward.nodeId,'a');});
