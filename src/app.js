...
cadParameterSection.addEventListener('change',event=>{const common=event.target.closest('[data-cargo-parameter]');if(common){layout.cargoSpec??={length:1.2,width:.8};layout.cargoSpec[common.dataset.cargoParameter]=Math.max(.1,Number(common.value)||.1);common.value=layout.cargoSpec[common.dataset.cargoParameter];resetEngine();return;}const input=event.target.closest('[data-equipment-id]');if(!input)return;const item=layout.equipment.find(entry=>entry.id===input.dataset.equipmentId);if(item){item.parameters??={};const integer=['levels','rows','columns','productTypes'].includes(input.dataset.parameter),value=Number(input.value);item.parameters[input.dataset.parameter]=integer?Math.max(1,Math.round(value||1)):value;if(input.dataset.parameter==='length')item.parameters.length=Math.max(.1,item.parameters.length||.1);if(input.dataset.parameter==='length'){item.source??={};item.source.parameterLengthUnit='m';}input.value=item.parameters[input.dataset.parameter];resetEngine();}});
