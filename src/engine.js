...
export class CadFlowEngine {
  ...
  asrsBatchConfig(){const storage=this.nodes.get(this.state.asrs.equipmentId),p=storage?.parameters||{};return{storage,enabled:Number(p.batchReleaseEnabled||0)!==0,size:Math.max(1,Math.round(Number(p.releaseBatchSize)||1))};}
  ...
  asrsInboundInterlocked(toNode){return toNode?.id===this.state.asrs.equipmentId&&this.state.cadTokens.some(token=>token.nodeId===toNode.id&&!token.edge&&['putaway','retrieval'].includes(token.asrsPhase));}
  holdAsrsInboundWhileBusy(){const storageId=this.state.asrs.equipmentId,busy=this.state.cadTokens.some(token=>token.nodeId===storageId&&!token.edge&&['putaway','retrieval'].includes(token.asrsPhase));if(!busy)return false;for(const token of this.state.cadTokens)if(token.edge?.to===storageId){token.edge=null;token.progress=1;token.blockedBy=storageId;token.queueState='ASRS작업대기';}return true;}
  holdAsrsInboundDuringRetrieval(){return this.holdAsrsInboundWhileBusy();}
  scheduleAsrsBatchRetrieval(){const {storage,enabled,size}=this.asrsBatchConfig();this.holdAsrsInboundWhileBusy();if(!storage||!enabled)return false;for(const zone of Object.values(this.state.asrs.zones))if(zone.releaseRemaining<=0&&zone.inventory>=size)zone.releaseRemaining=Math.min(size,zone.inventory);if(this.state.cadTokens.some(token=>token.nodeId===storage.id&&['putaway','retrieval'].includes(token.asrsPhase)))return false;const token=this.state.cadTokens.find(entry=>entry.nodeId===storage.id&&entry.asrsPhase==='stored'&&(this.state.asrs.zones[entry.flowKey]?.releaseRemaining||0)>0);if(!token)return false;const duration=cadDuration(storage,this.layout,{slotIndex:token.asrsTarget?.index||0,operation:'retrieval'});token.asrsPhase='retrieval';token.nodeEnteredAt=this.state.t;token.operationDuration=duration;token.readyAt=this.state.t+duration;this.emit('asrs-retrieval-start',{equipmentId:storage.id,zone:token.flowKey,target:token.asrsTarget,duration,batch:true});this.holdAsrsInboundWhileBusy();return true;}
  ...
  resolveZeroDelayHandovers(){this.holdAsrsInboundWhileBusy();let changed=true,guard=this.nodes.size+1;while(changed&&guard-->0){changed=false;for(const token of this.state.cadTokens)if(token.edge&&!this.asrsInboundInterlocked(this.nodes.get(token.edge.to))&&this.tryDirectHandover(token))changed=true;}}
