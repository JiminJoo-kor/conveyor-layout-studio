...
export class CadFlowEngine {
  ...
  prepareToken(token){let edge=token.edge??null,readyAt=token.readyAt??0,nodeId=token.nodeId;Object.defineProperty(token,'nodeId',{enumerable:true,configurable:true,get:()=>nodeId,set:value=>{nodeId=value;const item=this.nodes.get(value),enteredFromPrevious=token.handover?.targetId===value&&['conveyor','processLine','sorter'].includes(item?.type);token.motion=item?this.createMotion(item,enteredFromPrevious?cargoSpec(this.layout).length:0):null;token.motionState=token.motion?.controller.snapshot()||null;token.progress=token.motion?token.motionState.position/token.motion.distance:0;}});Object.defineProperty(token,'edge',{enumerable:true,configurable:true,get:()=>edge,set:value=>{if(value){const source=this.nodes.get(token.nodeId),duration=cargoSpec(this.layout).length/Math.max(.05,equipmentSpeedMetersPerSecond(source));token.handover={sourceId:token.nodeId,targetId:value.to,startedAt:this.state.t,endAt:this.state.t+duration,duration,visualOnly:true};}edge=value;}});Object.defineProperty(token,'readyAt',{enumerable:true,configurable:true,get:()=>readyAt,set:value=>{readyAt=value;}});return token;}
