...
function editorModeChanged(mode){$('connectEquipment').classList.toggle('active',mode.connecting);if(mode.placement){$('connectionHint').hidden=false;$('connectionHint').textContent=`${mode.placement.toUpperCase()} 배치 위치를 클릭하세요. Esc 또는 우클릭으로 취소할 수 있습니다.`;}else if(mode.connecting){const source=layout.equipment.find(item=>item.id===mode.sourceId);$('connectionHint').hidden=false;$('connectionHint').textContent=source?`${source.name}에서 이어질 대상 설비를 클릭하세요. 양옆 포트 중 가장 가까운 조합으로 연결합니다.`:'시작 설비와 도착 설비를 클릭하세요. 연결은 양옆 포트만 사용합니다.';}else if(!$('connectionHint').textContent.startsWith('연결 완료'))$('connectionHint').hidden=true;}
