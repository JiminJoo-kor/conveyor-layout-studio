...
function connectEquipment(fromId,toId,fromPort,toPort){if(fromId===toId)return false;layout.cadSchematic??={lanes:[],inboundBranches:[],edges:[]};layout.cadSchematic.edges??=[];if(layout.cadSchematic.edges.some(edge=>edge.from===fromId&&edge.to===toId&&edge.fromPort===fromPort&&edge.toPort===toPort))return false;const from=layout.equipment.find(item=>item.id===fromId),to=layout.equipment.find(item=>item.id===toId);if(!from||!to)return false;const ports=fromPort&&toPort?{fromPort,toPort}:closestPortPair(from,to),kind=connectionKind(from,to);layout.cadSchematic.edges.push({from:fromId,to:toId,kind,fromPort:ports.fromPort,toPort:ports.toPort,manual:true});$('connectionHint').hidden=false;$('connectionHint').textContent=`연결 완료: ${from.name} ${ports.fromPort} → ${to.name} ${ports.toPort}`;renderer.setLayout(layout);resetEngine();return true;}
...
function detectedFlowNames(){return [...new Set([...(layout.cadSchematic?.lanes||[]).map(line=>line.name),...(layout.cadSchematic?.inboundBranches||[]).map(line=>line.name),...Object.keys(engine.state?.asrs?.zones||{})].filter(Boolean))];}
function appendForkParameterControls(items){const flowNames=detectedFlowNames();for(const item of items.filter(entry=>entry.type==='forkingDevice')){item.parameters??={};const card=cadParameterSection.querySelector(`[data-parameter-card="${CSS.escape(item.id)}"]`),ratioInput=card?.querySelector('[data-parameter="output1Ratio"]'),ratioLabel=ratioInput?.closest('label');if(!card||!ratioLabel)continue;const enabled=item.parameters.distributionEnabled!==false,configured=Array.isArray(item.parameters.distributionFlowKeys)?item.parameters.distributionFlowKeys:[],selected=configured.length?configured:flowNames,role=document.createElement('label');role.className='fork-role-toggle';const roleText=document.createElement('span');roleText.textContent='분배 역할 사용';const checkbox=document.createElement('input');checkbox.type='checkbox';checkbox.checked=enabled;checkbox.dataset.forkEnabled='';checkbox.dataset.forkId=item.id;role.append(roleText,checkbox);ratioLabel.before(role);const selector=document.createElement('div');selector.className='fork-flow-selector';const title=document.createElement('strong');title.textContent='분배할 출발 라인 화살표';const hint=document.createElement('small');hint.textContent=enabled?'선택한 라인에서 온 물류만 직행/교차 순차 분배합니다.':'분배 역할이 꺼져 있어 모든 물류가 주 흐름으로 직행합니다.';const buttons=document.createElement('div');buttons.className='fork-flow-buttons';for(const name of flowNames){const button=document.createElement('button');button.type='button';button.textContent=name;button.dataset.forkFlow=name;button.dataset.forkId=item.id;button.classList.toggle('selected',selected.includes(name));button.disabled=!enabled;buttons.append(button);}selector.append(title,buttons,hint);ratioLabel.after(selector);}}
...
function renderCadEquipmentParameters(selectedId=selectedEquipment?.id){
  ...
  if(active){appendConveyorHandoverControls(items);appendForkParameterControls(items);appendCommonApplyControls(items);}
...
cadParameterSection.addEventListener('change',event=>{const checkbox=event.target.closest('[data-parameter-checkbox]');if(!checkbox)return;const item=layout.equipment.find(entry=>entry.id===checkbox.dataset.checkboxEquipmentId);if(!item)return;item.parameters??={};item.parameters[checkbox.dataset.parameterCheckbox]=checkbox.checked?1:0;resetEngine();});
cadParameterSection.addEventListener('change',event=>{const enabled=event.target.closest('[data-fork-enabled]');if(enabled){const item=layout.equipment.find(entry=>entry.id===enabled.dataset.forkId);if(item){item.parameters??={};item.parameters.distributionEnabled=enabled.checked;resetEngine();renderCadEquipmentParameters(item.id);}return;}const common=event.target.closest('[data-cargo-parameter]');if(common){layout.cargoSpec??={length:1200,width:800,weight:100,unit:'mm'};const cargoKey=common.dataset.cargoParameter;layout.cargoSpec[cargoKey]=cargoKey==='weight'?Math.max(.01,Number(common.value)||.01):Math.max(1,Math.round(Number(common.value)||1));layout.cargoSpec.unit='mm';common.value=layout.cargoSpec[common.dataset.cargoParameter];resetEngine();return;}const input=event.target.closest('[data-equipment-id]');if(!input||input.readOnly)return;const item=layout.equipment.find(entry=>entry.id===input.dataset.equipmentId);if(item){item.parameters??={};const binary=['simultaneousMotion','returnHome','motionProfile'].includes(input.dataset.parameter),integer=binary||['levels','rows','columns','productTypes','stackerCount'].includes(input.dataset.parameter),value=Number(input.value);item.parameters[input.dataset.parameter]=integer?Math.max(binary?0:1,Math.round(value||0)):value;if(input.dataset.parameter==='length')item.parameters.length=Math.max(.1,item.parameters.length||.1);if(input.dataset.parameter==='length'){item.source??={};item.source.parameterLengthUnit='m';}input.value=item.parameters[input.dataset.parameter];if(engine instanceof CadFlowEngine)engine.hotReloadEquipment(item);else resetEngine();renderer.draw(engine.state);if(['levels','rows','columns','productTypes','stackerCount'].includes(input.dataset.parameter)){resetEngine();renderCadEquipmentParameters(item.id);}}});
cadParameterSection.addEventListener('click',event=>{const button=event.target.closest('[data-apply-equipment]');if(!button)return;const source=layout.equipment.find(entry=>entry.id===button.dataset.applyEquipment);if(!source)return;const keys=button.dataset.applyParameter?[button.dataset.applyParameter]:Object.keys(source.parameters||{});for(const target of layout.equipment.filter(entry=>entry.id!==source.id&&entry.type===source.type)){target.parameters??={};for(const key of keys)if(source.parameters?.[key]!==undefined)target.parameters[key]=structuredClone(source.parameters[key]);if(source.type==='conveyor'&&keys.includes('length')){target.source??={};target.source.parameterLengthUnit='m';}}resetEngine();renderCadEquipmentParameters(source.id);});
cadParameterSection.addEventListener('click',event=>{const button=event.target.closest('[data-fork-flow]');if(!button)return;const item=layout.equipment.find(entry=>entry.id===button.dataset.forkId);if(!item)return;const names=detectedFlowNames();item.parameters??={};const selected=new Set(Array.isArray(item.parameters.distributionFlowKeys)&&item.parameters.distributionFlowKeys.length?item.parameters.distributionFlowKeys:names);selected.has(button.dataset.forkFlow)?selected.delete(button.dataset.forkFlow):selected.add(button.dataset.forkFlow);item.parameters.distributionFlowKeys=[...selected];resetEngine();renderCadEquipmentParameters(item.id);});
