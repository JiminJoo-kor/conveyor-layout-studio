...
export class LayoutRenderer {
  ...
  drawCadSymbol(item,state){
    ...
    if(['stackerCrane','asrs'].includes(item.type)){const p=item.parameters||{},levels=Math.max(1,Math.round(Number(p.levels)||4)),rows=Math.max(1,Math.round(Number(p.rows)||3)),columns=Math.max(1,Math.round(Number(p.columns)||4)),productTypes=Math.max(1,Math.round(Number(p.productTypes)||3)),w=compact?70:150,h=compact?58:112,zones=Object.entries(state?.asrs?.zones||{}),drawColumns=Math.min(columns,10),drawLevels=Math.min(levels,8),rackLeft=-w*.46,rackTop=-h*.43,rackWidth=w*.78,rackHeight=h*.68,cellW=rackWidth/drawColumns,cellH=rackHeight/drawLevels,palette=['rgba(0,212,255,.35)','rgba(0,255,136,.35)','rgba(255,210,63,.38)','rgba(255,77,157,.35)','rgba(255,113,57,.35)'];c.save();c.translate(item.x,item.y);c.fillStyle='#0d2530';c.strokeStyle=COLORS.green;c.lineWidth=1;c.fillRect(-w/2,-h/2,w,h);c.strokeRect(-w/2,-h/2,w,h);for(let row=0;row<drawLevels;row++)for(let column=0;column<drawColumns;column++){const zoneIndex=Math.min(productTypes-1,Math.floor(row/drawLevels*productTypes));c.fillStyle=palette[zoneIndex%palette.length];c.fillRect(rackLeft+column*cellW,rackTop+row*cellH,cellW,cellH);c.strokeStyle='rgba(216,243,255,.35)';c.strokeRect(rackLeft+column*cellW,rackTop+row*cellH,cellW,cellH);}c.strokeStyle=COLORS.orange;c.lineWidth=compact?2:3;c.beginPath();c.moveTo(w*.38,rackTop);c.lineTo(w*.38,rackTop+rackHeight);c.stroke();c.fillStyle=COLORS.orange;c.fillRect(w*.38-3,-3,6,6);c.fillStyle='#d8f3ff';c.font=`${compact?6:8}px monospace`;c.fillText(`${levels}층×${columns}열×${rows}행 · ${productTypes}품목`,-w*.46,h*.36);c.fillText(`${state?.asrs?.inventory||0}/${state?.asrs?.capacity||levels*rows*columns}`,-w*.46,h*.47);if(!compact)zones.slice(0,5).forEach(([name,zone],index)=>{c.fillStyle=palette[index%palette.length].replace('.35','.9').replace('.38','.9');c.fillRect(-w*.46+index*w*.18,h*.4,5,5);c.fillStyle='#d8f3ff';c.font='6px monospace';c.fillText(`${name.replace(' 라인','')} ${zone.inventory}/${zone.capacity}`,-w*.42+index*w*.18,h*.46);});c.restore();return true;}
  ...
  drawCadFlow(state){
    ...
    if(state.cadTokens){const nodes=new Map(this.layout.equipment.filter(item=>item.source?.origin==='dxf').map(item=>[item.id,item]));for(const token of state.cadTokens){const fromNode=nodes.get(token.edge?.from||token.nodeId),toNode=nodes.get(token.edge?.to||token.nodeId);if(!fromNode||!toNode)continue;const from=connectionAnchor(fromNode,token.edge?.fromPort),to=connectionAnchor(toNode,token.edge?.toPort),point=pointOnRoute(orthogonalRoute(from,to),token.edge?token.progress:0),x=point.x,y=point.y;c.fillStyle=COLORS.yellow;c.strokeStyle='#fff2a8';c.fillRect(x-7,y-7,14,14);c.strokeRect(x-7,y-7,14,14);}return;}
