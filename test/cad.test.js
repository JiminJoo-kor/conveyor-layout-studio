...
test('컨베이어는 화면 크기와 분리된 실제 길이 파라미터를 제공한다',()=>{
  const fields=parameterFieldsFor({type:'conveyor',length:90,parameters:{speed:.5,beltWidth:2}}),length=fields.find(field=>field.key==='length'),cargoLength=fields.find(field=>field.key==='cargoLength'),cargoWidth=fields.find(field=>field.key==='cargoWidth'),speed=fields.find(field=>field.key==='speed');assert.equal(length.value,5);assert.equal(length.label,'실제 길이(m)');assert.equal(cargoLength.value,1.2);assert.equal(cargoLength.label,'물류 길이(m)');assert.equal(cargoWidth.value,.8);assert.equal(cargoWidth.label,'물류 폭(m)');assert.equal(speed.label,'속도(m/s)');assert.equal(fields.some(field=>field.key==='beltWidth'),false);
});
