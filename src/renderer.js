...
import { itemVisualLength, rigidHandoverVisualState } from './occupancy.js';
...
export function conveyorCargoVisualPose(item,cargo,physicalPosition,commonVisualLength=78){const physicalLength=Math.max(.1,Number(item?.parameters?.length)||cargo.length),scale=commonVisualLength/physicalLength,angle=(Number(item?.rotation)||0)*Math.PI/180,local=-commonVisualLength/2+(Math.max(0,Number(physicalPosition)||0)-cargo.length/2)*scale;return{x:item.x+Math.cos(angle)*local,y:item.y+Math.sin(angle)*local,angle,visualLength:Math.max(1,cargo.length*scale),visualWidth:Math.max(5,Math.min(24,cargo.width*24/physicalLength)),scale};}
...
export class LayoutRenderer {
  ...
  handoverDescriptor(token,state,nodes,cargo){const common=item=>item.type==='conveyor'?(this.layout.cadViewMode==='hybrid'?58:78):44,physical=item=>Math.max(.1,Number(item.parameters?.length)||cargo.length);if(!token.edge&&token.motion&&['conveyor','processLine','sorter'].includes(nodes.get(token.nodeId)?.type)){const source=nodes.get(token.nodeId),position=Number(token.motionState?.position)||0,entered=Math.max(0,position-physical(source)),outgoing=(this.layout.cadSchematic?.edges||[]).filter(edge=>edge.from===source.id);if(entered>0&&entered<cargo.length&&outgoing.length===1)return{source,target:nodes.get(outgoing[0].to),edge:outgoing[0],raw:entered/cargo.length};}const handover=token.handover,source=nodes.get(handover?.sourceId),target=nodes.get(handover?.targetId);if(!handover||!source||!target)return null;const targetPosition=token.nodeId===target.id?Number(token.motionState?.position):NaN,raw=Number.isFinite(targetPosition)?targetPosition/cargo.length:(state.t-handover.startedAt)/Math.max(.001,handover.duration);return raw<1?{source,target,edge:token.edge||{},raw:Math.max(0,raw)}:null;}

  drawHandoverOverlay(state){const c=this.ctx,nodes=new Map(this.layout.equipment.map(item=>[item.id,item])),cargo=normalizedCargoSpec(this.layout),visualSize=item=>item.type==='conveyor'?(this.layout.cadViewMode==='hybrid'?58:78):44,physicalSize=item=>Math.max(.1,Number(item.parameters?.length)||cargo.length),speed=item=>Math.max(.05,Number(item.parameters?.speed)||Number(item.parameters?.travelSpeed)||1);for(const token of state.cadTokens||[]){const descriptor=this.handoverDescriptor(token,state,nodes,cargo);if(!descriptor?.source||!descriptor?.target)continue;const {source,target,edge,raw}=descriptor,sourceScale=visualSize(source)/physicalSize(source),targetScale=visualSize(target)/physicalSize(target),sourceVelocity=speed(source)*sourceScale,targetVelocity=speed(target)*targetScale,visual=rigidHandoverVisualState(raw,cargo.length,sourceScale,targetScale,sourceVelocity,targetVelocity),sourcePose=conveyorCargoVisualPose(source,cargo,physicalSize(source),visualSize(source)),targetPose=conveyorCargoVisualPose(target,cargo,cargo.length,visualSize(target)),route=edgeRoute({x:sourcePose.x,y:sourcePose.y},{x:targetPose.x,y:targetPose.y},edge),point=pointOnRoute(route,visual.routeProgress),before=pointOnRoute(route,Math.max(0,visual.routeProgress-.005)),after=pointOnRoute(route,Math.min(1,visual.routeProgress+.005)),angle=Math.atan2(after.y-before.y,after.x-before.x),height=Math.max(5,Math.min(18,cargo.width*((24/physicalSize(source))*(1-visual.targetWeight)+(24/physicalSize(target))*visual.targetWeight))),color=flowColor(token.flowKey,token.flowIndex);c.save();c.beginPath();for(let index=1;index<route.length;index++){const a=route[index-1],b=route[index],padding=Math.max(10,height);c.rect(Math.min(a.x,b.x)-padding,Math.min(a.y,b.y)-padding,Math.abs(b.x-a.x)+padding*2,Math.abs(b.y-a.y)+padding*2);}c.clip();c.translate(point.x,point.y);c.rotate(angle);c.fillStyle=color;c.strokeStyle='rgba(255,255,255,.85)';c.lineWidth=1;c.fillRect(-visual.visualLength/2,-height/2,visual.visualLength,height);c.strokeRect(-visual.visualLength/2,-height/2,visual.visualLength,height);c.restore();}}

  drawCadFlow(state){
    const c=this.ctx,conveyors=this.layout.equipment.filter(item=>item.type==='conveyor'&&item.source?.origin==='dxf');
    if(state.cadTokens){
      const nodes=new Map(this.layout.equipment.filter(item=>item.source?.origin==='dxf').map(item=>[item.id,item])),cargo=normalizedCargoSpec(this.layout);
