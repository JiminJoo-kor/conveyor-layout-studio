...
export const equipmentParameterLabels = {
  ...
  travelSpeed:'주행 속도(m/s)',liftSpeed:'승강 속도(m/s)',loadCapacity:'적재 하중(kg)',lineSpeed:'라인 속도(m/min)',pitch:'차체 피치(m)',bufferCapacity:'라인 버퍼 수',transferTime:'H/P 이재시간(초)',forkTime:'포킹 동작시간(초)',strokeDistance:'포크 스트로크(m)'
...
export function buildSchematicLayout(candidates){
  ...
  if(labels.length>=2){for(const label of [...labels].sort((a,b)=>a.y-b.y))lanes.push({id:`lane-${lanes.length+1}`,name:label.name,labelId:label.id,y:label.y,nodes:[]});const inboundDocks=flowNodes.filter(item=>item.type==='dock'&&/TRUCK|INBOUND|입고/i.test(item.name||'')),regularNodes=flowNodes.filter(item=>!inboundDocks.includes(item));for(const node of regularNodes){const lane=[...lanes].sort((a,b)=>Math.abs(a.y-node.y)-Math.abs(b.y-node.y))[0];lane.nodes.push(node);}const sortedDocks=[...inboundDocks].sort((a,b)=>a.y-b.y),sortedLanes=[...lanes].sort((a,b)=>a.y-b.y);sortedLanes.forEach((lane,index)=>{const dock=sortedDocks[index];if(dock)lane.nodes.push(dock);});}
  ...
  lanes.forEach((lane,index)=>{
    lane.name=lane.name||`${index+1}번 라인`;const inboundDock=lane.nodes.find(item=>item.type==='dock'&&/TRUCK|INBOUND|입고/i.test(item.name||'')),source=lane.nodes.find(item=>item.type==='source')||inboundDock,sink=lane.nodes.find(item=>item.type==='sink'||(item.type==='dock'&&item!==inboundDock));
    ...
    for(let i=1;i<lane.nodes.length;i++){const from=lane.nodes[i-1],to=lane.nodes[i],minX=Math.min(from.x,to.x),maxX=Math.max(from.x,to.x),bridge=labels.length<2?bridges.filter(item=>!usedBridges.has(item.id)&&item.x>=minX&&item.x<=maxX).sort((a,b)=>Math.abs(a.y-lane.y)-Math.abs(b.y-lane.y))[0]:null;if(bridge&&Math.abs(bridge.y-lane.y)<180){edges.push({from:from.id,to:bridge.id,kind:'transfer'},{from:bridge.id,to:to.id,kind:'transfer'});usedBridges.add(bridge.id);laneHasBridge=true;}else edges.push({from:from.id,to:to.id,kind:'flow'});}
...
export function normalizeSchematicPositions(candidates,schematic,width=1200){
  if(schematic.lanes.length>=3&&schematic.lanes.every(lane=>lane.labelId)){
    ...
    for(const bridge of candidates.filter(item=>['agv','amr'].includes(item.type)))positionById.set(bridge.id,{x:870,y:laneYFor(bridge)});
...
export async function analyzeCadFile(file) {
  ...
  const document=parseDxf(await file.text()),allCandidates=buildLayoutCandidates(document),processRegion=detectProcessRegion(document),regionCandidates=processRegion?allCandidates.filter(item=>item.type!=='unknown'&&inside({x:item.x,y:item.y},processRegion)):null,nearbyTrucks=processRegion?allCandidates.filter(item=>item.type==='dock'&&item.y>=processRegion.minY&&item.y<=processRegion.maxY&&item.x<processRegion.minX&&processRegion.minX-item.x<120000):[],cluster=dedupeProcessLineCandidates(processRegion?[...regionCandidates,...nearbyTrucks]:selectPrimaryLayoutCluster(allCandidates)),points=cluster.flatMap(item=>[{x:item.x-item.width/2,y:item.y-item.height/2},{x:item.x+item.width/2,y:item.y+item.height/2}]);
