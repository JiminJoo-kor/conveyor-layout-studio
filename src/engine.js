...
export class CadFlowEngine {
  ...
  asrsInboundInterlocked(toNode,tokenOrLine){const lineKey=typeof tokenOrLine==='string'?tokenOrLine:tokenOrLine?.flowKey;if(toNode?.id!==this.state.asrs.equipmentId)return false;if(this.asrsLineBusy(lineKey,typeof tokenOrLine==='object'?tokenOrLine:null))return true;return this.ensureAsrsLineMode(lineKey,'inbound');}
  holdAsrsInboundWhileBusy(){const storageId=this.state.asrs.equipmentId;let held=false;for(const token of this.state.cadTokens)if(token.edge?.to===storageId&&this.asrsLineBusy(token.flowKey,token)){token.edge=null;token.progress=1;token.blockedBy=storageId;token.queueState='ASRS작업대기';held=true;}return held;}
