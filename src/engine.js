...
export const equipmentLengthMeters=(item,layout)=>{const configured=Number(item?.parameters?.length),scale=unitScale(layout?.cadSource?.units);if(Number.isFinite(configured)&&configured>0)return item?.source?.origin==='dxf'&&scale?configured*scale:configured;return Math.max(.1,Number(item?.length||item?.width||20)/20);};
