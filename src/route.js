...
export function equipmentPorts(item,distance=null){
  const storage=['asrs','stackerCrane'].includes(item?.type),horizontal=Number.isFinite(distance)?distance:item?.asrsStation?item.asrsStation.visualLength/2:storage?97:44,vertical=Number.isFinite(distance)?distance:item?.asrsStation?item.asrsStation.visualWidth/2:storage?74:44,angle=(Number(item?.rotation)||0)*Math.PI/180,rotate=(x,y)=>({x:item.x+x*Math.cos(angle)-y*Math.sin(angle),y:item.y+x*Math.sin(angle)+y*Math.cos(angle)}),ports={left:rotate(-horizontal,0),right:rotate(horizontal,0),top:rotate(0,-vertical),bottom:rotate(0,vertical)};
