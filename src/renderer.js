...
export class LayoutRenderer {
  ...
  handoverDescriptor(token,state,nodes,cargo){const physical=item=>Math.max(.1,Number(item.parameters?.length)||cargo.length);if(!token.edge&&token.motion&&['conveyor','processLine','sorter'].includes(nodes.get(token.nodeId)?.type)){const source=nodes.get(token.nodeId),position=Number(token.motionState?.position)||0,entered=Math.max(0,position-physical(source)),outgoing=(this.layout.cadSchematic?.edges||[]).filter(edge=>edge.from===source.id),edge=token.visualEdge||outgoing[0],target=nodes.get(edge?.to);if(entered>0&&entered<cargo.length&&edge&&target)return{source,target,edge,raw:entered/cargo.length};}if(token.edge)return null;const handover=token.handover,source=nodes.get(handover?.sourceId),target=nodes.get(handover?.targetId);if(!handover||!source||!target)return null;if(token.nodeId===target.id&&!token.motion)return null;const targetPosition=token.nodeId===target.id?Number(token.motionState?.position):NaN,raw=Number.isFinite(targetPosition)?targetPosition/cargo.length:(state.t-handover.startedAt)/Math.max(.001,handover.duration);return raw<1?{source,target,edge:handover.edge||{},raw:Math.max(0,raw)}:null;}

  drawHandoverOverlay(state){const c=this.ctx,nodes=new Map(this.layout.equipment.map(item=>[item.id,item])),cargo=normalizedCargoSpec(this.layout),commonVisualLength=this.layout.cadViewMode==='hybrid'?58:78,metrics=stableCargoVisualMetrics(this.layout,cargo,commonVisualLength),colorFor=token=>flowColor(token.flowKey,token.flowIndex),drawEndpoint=(item,port,other,progress,isTarget,color)=>{const bounds=equipmentClipBounds(item,commonVisualLength),cargoLength=metrics.visualLength,cargoWidth=metrics.visualWidth,pose=handoverEndpointPose(item,port,cargoLength,isTarget,other,progress,commonVisualLength),itemAngle=(Number(item.rotation)||0)*Math.PI/180;c.save();c.translate(item.x,item.y);c.rotate(itemAngle);c.beginPath();c.rect(-bounds.width/2,-bounds.height/2,bounds.width,bounds.height);c.clip();c.rotate(-itemAngle);c.translate(-item.x,-item.y);c.translate(pose.x,pose.y);c.rotate(pose.angle);c.fillStyle=color;c.strokeStyle='rgba(255,255,255,.85)';c.lineWidth=1;c.fillRect(-cargoLength/2,-cargoWidth/2,cargoLength,cargoWidth);c.strokeRect(-cargoLength/2,-cargoWidth/2,cargoLength,cargoWidth);c.restore();};for(const token of state.cadTokens||[]){const descriptor=this.handoverDescriptor(token,state,nodes,cargo);if(!descriptor?.source||!descriptor?.target)continue;const raw=Math.max(0,Math.min(1,descriptor.raw)),color=colorFor(token),edge=descriptor.edge||{};drawEndpoint(descriptor.source,edge.fromPort,descriptor.target,raw,false,color);drawEndpoint(descriptor.target,edge.toPort,descriptor.source,raw,true,color);}}
  ...
  drawCadFlow(state){
    ...
    if(state.cadTokens){
      ...
      for(const token of state.cadTokens){
        ...
        else if(!token.edge&&['asrs','stackerCrane'].includes(fromNode.type)){const incoming=(this.layout.cadSchematic?.edges||[]).find(edge=>edge.to===fromNode.id),outgoing=(this.layout.cadSchematic?.edges||[]).find(edge=>edge.from===fromNode.id),retrieval=token.asrsPhase==='retrieval',edge=retrieval?outgoing:incoming,other=nodes.get(retrieval?edge?.to:edge?.from);point=retrieval?handoverEndpointPose(fromNode,edge?.fromPort,cargoW,false,other,0,this.layout.cadViewMode==='hybrid'?58:78):handoverEndpointPose(fromNode,edge?.toPort,cargoW,true,other,1,this.layout.cadViewMode==='hybrid'?58:78);cargoAngle=point.angle;clipItem=fromNode;}
        else if(!token.edge)point={x:fromNode.x,y:fromNode.y};
        else{point=pendingTransferPose(fromNode,toNode,token.edge,cargoW,this.layout.cadViewMode==='hybrid'?58:78);cargoAngle=point.angle;clipItem=fromNode;}
        const x=point.x,y=point.y;c.save();if(clipItem){const bounds=equipmentClipBounds(clipItem,this.layout.cadViewMode==='hybrid'?58:78),itemAngle=(Number(clipItem.rotation)||0)*Math.PI/180;c.translate(clipItem.x,clipItem.y);c.rotate(itemAngle);c.beginPath();c.rect(-bounds.width/2,-bounds.height/2,bounds.width,bounds.height);c.clip();c.rotate(-itemAngle);c.translate(-clipItem.x,-clipItem.y);}c.translate(x,y);c.rotate(cargoAngle);c.fillStyle=flowColor(token.flowKey,token.flowIndex);c.strokeStyle='rgba(255,255,255,.85)';c.fillRect(-cargoW/2,-cargoH/2,cargoW,cargoH);c.strokeRect(-cargoW/2,-cargoH/2,cargoW,cargoH);c.restore();
      }
      return;
    }
