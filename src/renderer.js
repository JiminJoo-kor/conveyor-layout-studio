...
export class LayoutRenderer {
  ...
  drawCadFlow(state){
    ...
    if(state.cadTokens){const nodes=new Map(this.layout.equipment.filter(item=>item.source?.origin==='dxf').map(item=>[item.id,item]));for(const token of state.cadTokens){const fromNode=nodes.get(token.edge?.from||token.nodeId),toNode=nodes.get(token.edge?.to||token.nodeId);if(!fromNode||!toNode)continue;let point;if(!token.edge&&['agv','amr'].includes(fromNode.type)){const route=mobileEquipmentRoute(this.layout,fromNode),operation=equipmentOperationProgress(fromNode,state);point=route?pointOnRoute(route.points||[route.start,route.end],operation.progress):{x:fromNode.x,y:fromNode.y};}else if(!token.edge&&fromNode.type==='conveyor'){const operation=equipmentOperationProgress(fromNode,state),visualLength=this.layout.cadViewMode==='hybrid'?58:78,angle=(fromNode.rotation||0)*Math.PI/180,offset=-visualLength/2+operation.progress*visualLength;point={x:fromNode.x+Math.cos(angle)*offset,y:fromNode.y+Math.sin(angle)*offset};}else{const from=connectionAnchor(fromNode,token.edge?.fromPort),to=connectionAnchor(toNode,token.edge?.toPort);point=pointOnRoute(orthogonalRoute(from,to),token.edge?token.progress:0);}const x=point.x,y=point.y;c.fillStyle=COLORS.yellow;c.strokeStyle='#fff2a8';c.fillRect(x-7,y-7,14,14);c.strokeRect(x-7,y-7,14,14);}return;}
