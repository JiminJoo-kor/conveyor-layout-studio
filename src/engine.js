...
export const cadDuration=(item,layout,context={})=>{
  ...
  else if(['agv','amr','shuttle'].includes(item?.type))duration=Number(p.shuttleDistance||5)/Math.max(.1,Number(p.speed||1.2))+Number(p.loadTime||2)+Number(p.unloadTime||2);
  ...
  else if(item?.type==='forkingDevice')duration=Number(p.forkTime||4);
...
export class CadFlowEngine {
  ...
  createMotion(item){const distance=['conveyor','processLine','sorter'].includes(item.type)?equipmentLengthMeters(item,this.layout)+cargoSpec(this.layout).length:['agv','amr','shuttle'].includes(item.type)?Math.max(.1,Number(item.parameters?.shuttleDistance)||5):0;return distance>0?{controller:new KinematicMotion(motionConfigFor(item)),distance}:null;}
  advanceMotion(token,item,dt,blocked=false){if(!token.motion)return true;token.motion.controller.updateConfig(motionConfigFor(item));const reliability=this.reliabilityFor(item).step(dt,item.parameters),occupancy=this.state.cadTokens.filter(other=>other!==token&&!other.edge&&other.nodeId===item.id).length+1,speedFactor=equipmentLoadSpeedFactor(item,this.layout,occupancy),snapshot=token.motion.controller.step(dt,{distance:token.motion.distance,blocked:blocked||!reliability.available,speedFactor});token.motionState=snapshot;token.progress=Math.min(1,snapshot.position/token.motion.distance);return snapshot.position>=token.motion.distance;}
  ...
  prepareToken(token){let edge=token.edge??null,readyAt=token.readyAt??0,nodeId=token.nodeId;Object.defineProperty(token,'nodeId',{enumerable:true,configurable:true,get:()=>nodeId,set:value=>{nodeId=value;const item=this.nodes.get(value);token.motion=item?this.createMotion(item):null;}});Object.defineProperty(token,'edge',{enumerable:true,configurable:true,get:()=>edge,set:value=>{if(value){const source=this.nodes.get(token.nodeId),duration=cargoSpec(this.layout).length/Math.max(.05,equipmentSpeedMetersPerSecond(source));token.handover={sourceId:token.nodeId,targetId:value.to,startedAt:this.state.t,endAt:this.state.t+duration,duration};}edge=value;}});Object.defineProperty(token,'readyAt',{enumerable:true,configurable:true,get:()=>Math.max(readyAt,token.handover?.endAt||0),set:value=>{readyAt=value;}});return token;}
