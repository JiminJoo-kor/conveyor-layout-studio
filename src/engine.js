...
export class CadFlowEngine {
  ...
  createMotion(item,initialPosition=0){const distance=['conveyor','processLine','sorter'].includes(item.type)?equipmentLengthMeters(item,this.layout)+cargoSpec(this.layout).length:['agv','amr','shuttle'].includes(item.type)?Math.max(.1,Number(item.parameters?.shuttleDistance)||5):0;return distance>0?{controller:new KinematicMotion(motionConfigFor(item),{position:Math.min(distance,Math.max(0,initialPosition))}),distance}:null;}
  ...
  prepareToken(token){let edge=token.edge??null,readyAt=token.readyAt??0,nodeId=token.nodeId;Object.defineProperty(token,'nodeId',{enumerable:true,configurable:true,get:()=>nodeId,set:value=>{const previous=nodeId,transfer=edge&&edge.to===value?{from:previous,to:value}:null;nodeId=value;if(transfer){this.emit('sensor-outfeed',{equipmentId:transfer.from,productId:token.id,transferTo:transfer.to});this.emit('sensor-infeed',{equipmentId:transfer.to,productId:token.id,transferFrom:transfer.from});}const item=this.nodes.get(value),enteredFromPrevious=token.handover?.targetId===value&&['conveyor','processLine','sorter'].includes(item?.type),entryPosition=enteredFromPrevious?conveyorEntryPosition(item,this.layout):0;token.motion=item?this.createMotion(item,entryPosition):null;token.motionState=token.motion?.controller.snapshot()||null;token.progress=token.motion?token.motionState.position/token.motion.distance:0;token.visualEdge=null;}});Object.defineProperty(token,'edge',{enumerable:true,configurable:true,get:()=>edge,set:value=>{if(value){const source=this.nodes.get(token.nodeId),duration=cargoSpec(this.layout).length/Math.max(.05,equipmentSpeedMetersPerSecond(source));token.handover={sourceId:token.nodeId,targetId:value.to,edge:value,startedAt:this.state.t,endAt:this.state.t+duration,duration,visualOnly:true};}edge=value;}});Object.defineProperty(token,'readyAt',{enumerable:true,configurable:true,get:()=>readyAt,set:value=>{readyAt=value;}});return token;}
