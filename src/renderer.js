...
export function conveyorVisualPositionMap(tokens,item,layout,cargo,metrics,commonVisualLength=78){const scale=commonVisualLength/Math.max(.1,equipmentLengthMeters(item,layout)),gapMeters=Math.max(0,Number(item?.parameters?.safetyGap)||0),visibleGap=Math.max(3,gapMeters*scale),required=(metrics.visualLength+visibleGap)/scale,ordered=[...tokens].sort((a,b)=>(Number(b.motionState?.position)||0)-(Number(a.motionState?.position)||0)),positions=new Map();let leader=Infinity;for(const token of ordered){const physical=Math.max(0,Number(token.motionState?.position)||0),display=Number.isFinite(leader)?Math.min(physical,leader-required):physical;positions.set(token,display);leader=display;}return positions;}
