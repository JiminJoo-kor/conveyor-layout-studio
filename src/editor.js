...
export class LayoutEditor {
  ...
  pointerDown(event){if(!this.enabled)return;this.canvas.setPointerCapture(event.pointerId);const p=this.worldPoint(event),hit=this.hit(p);
    if(event.button===1||event.shiftKey){const raw=this.canvasPoint(event);this.pan={raw,start:{...this.view}};return;}
  ...
  pointerMove(event){if(!this.enabled)return;if(this.pan){const p=this.canvasPoint(event);this.view.x=this.pan.start.x+p.x-this.pan.raw.x;this.view.y=this.pan.start.y+p.y-this.pan.raw.y;this.renderer.setView(this.view);this.onChange(false);return;}
  ...
  pointerUp(){if(this.drag)this.onChange(true);this.drag=null;this.pan=null;}
  wheel(event){if(!this.enabled)return;event.preventDefault();const before=this.worldPoint(event),factor=event.deltaY<0?1.1:.9;this.view.zoom=Math.max(.5,Math.min(2.5,this.view.zoom*factor));const raw=this.canvasPoint(event);this.view.x=raw.x-before.x*this.view.zoom;this.view.y=raw.y-before.y*this.view.zoom;this.renderer.setView(this.view);this.onChange(false);}
