...
export class CadFlowEngine {
  ...
  updateTokenLineMembership(){for(const token of this.state.cadTokens){if(token.nodeId===this.state.asrs.equipmentId)continue;const membership=this.lineMembershipForNode(token.nodeId);if(!membership?.name||membership.name===token.flowKey)continue;const previous=token.flowKey;token.originFlowKey??=previous;token.cargoType??=token.originFlowKey;token.flowKey=membership.name;token.flowIndex=membership.index;this.emit('cargo-line-changed',{productId:token.id,from:previous,to:membership.name,cargoType:token.cargoType,equipmentId:token.nodeId});}}
