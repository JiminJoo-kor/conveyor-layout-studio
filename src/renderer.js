...
export function handoverEndpointPose(item,port,cargoLength,isTarget=false,other=null){const ports=equipmentPorts(item),resolved=port&&ports[port]?port:Object.entries(ports).sort((a,b)=>Math.hypot(a[1].x-(other?.x??item.x),a[1].y-(other?.y??item.y))-Math.hypot(b[1].x-(other?.x??item.x),b[1].y-(other?.y??item.y)))[0]?.[0],anchor=ports[resolved]||{x:item.x,y:item.y},distance=Math.max(1,Math.hypot(anchor.x-item.x,anchor.y-item.y)),outward={x:(anchor.x-item.x)/distance,y:(anchor.y-item.y)/distance},movement=isTarget?{x:-outward.x,y:-outward.y}:outward;return{x:anchor.x-outward.x*cargoLength/2,y:anchor.y-outward.y*cargoLength/2,angle:Math.atan2(movement.y,movement.x),port:resolved};}
...
export class LayoutRenderer {
  ...
  drawHandoverOverlay(state){const c=this.ctx,nodes=new Map(this.layout.equipment.map(item=>[item.id,item])),cargo=normalizedCargoSpec(this.layout),commonVisualLength=this.layout.cadViewMode==='hybrid'?58:78,metrics=stableCargoVisualMetrics(this.layout,cargo,commonVisualLength),visualSize=item=>item.type==='conveyor'?commonVisualLength:item.type==='forkingDevice'?58:44,colorFor=token=>flowColor(token.flowKey,token.flowIndex),drawEndpoint=(item,port,other,fraction,isTarget,color)=>{if(!(fraction>0))return;const cargoLength=Math.min(metrics.visualLength,visualSize(item)*.9),cargoWidth=metrics.visualWidth,visible=cargoLength*Math.min(1,fraction),pose=handoverEndpointPose(item,port,cargoLength,isTarget,other);c.save();c.translate(pose.x,pose.y);c.rotate(pose.angle);c.beginPath();c.rect(-cargoLength/2,-cargoWidth/2,visible,cargoWidth);c.clip();c.fillStyle=color;c.strokeStyle='rgba(255,255,255,.85)';c.lineWidth=1;c.fillRect(-cargoLength/2,-cargoWidth/2,cargoLength,cargoWidth);c.strokeRect(-cargoLength/2,-cargoWidth/2,cargoLength,cargoWidth);c.restore();};for(const token of state.cadTokens||[]){const descriptor=this.handoverDescriptor(token,state,nodes,cargo);if(!descriptor?.source||!descriptor?.target)continue;const raw=Math.max(0,Math.min(1,descriptor.raw)),color=colorFor(token),edge=descriptor.edge||{};drawEndpoint(descriptor.source,edge.fromPort,descriptor.target,1-raw,false,color);drawEndpoint(descriptor.target,edge.toPort,descriptor.source,raw,true,color);}}
