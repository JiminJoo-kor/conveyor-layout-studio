...
const cadDuration=item=>{if(['asrs','stackerCrane'].includes(item?.type)){const p=item.parameters||{},travel=12/Math.max(.1,Number(p.travelSpeed||2.5)),lift=Math.max(1,Number(p.levels||1))*1.5/Math.max(.1,Number(p.liftSpeed||1));return Math.max(.2,travel+lift+4);}return Math.max(.2,Number(item?.parameters?.cycleTime??item?.parameters?.processTime??item?.parameters?.dischargeTime??1));};
export class CadFlowEngine {
  ...
  reset(){const storage=[...this.nodes.values()].find(node=>['asrs','stackerCrane'].includes(node.type)),p=storage?.parameters||{},capacity=Math.max(1,Number(p.rows||2)*Number(p.columns||10)*Number(p.levels||1));this.state={t:0,cadTokens:[],nextId:1,nextInjection:0,completedProducts:[],events:[],movedItems:0,locks:{},robot:{phase:'idle'},source:[],product:[],asrs:{equipmentId:storage?.id,inventory:0,capacity,putaways:0,retrievals:0,busyTime:0}};return this.state;}
  ...
  step(dt){const s=this.state;if(!(dt>0)||s.t>=this.params.simDuration)return s;s.t+=Math.min(dt,this.params.simDuration-s.t);if(s.t>=s.nextInjection){for(const source of this.sources)s.cadTokens.push({id:s.nextId++,nodeId:source.id,createdAt:s.t,readyAt:s.t+cadDuration(source),edge:null,progress:0});s.nextInjection=s.t+Math.max(1,this.params.injectA||30);this.emit('source-injected',{equipmentId:this.sources.map(x=>x.id).join(',')});}
    for(const token of [...s.cadTokens]){if(token.nodeId===s.asrs.equipmentId&&!token.edge)s.asrs.busyTime+=dt;if(token.edge){const from=this.nodes.get(token.edge.from),to=this.nodes.get(token.edge.to),distance=Math.max(40,Math.hypot(to.x-from.x,to.y-from.y)),lineSpeed=Number(from.parameters?.lineSpeed),speed=Math.max(.2,Number(from.parameters?.speed??(Number.isFinite(lineSpeed)?lineSpeed/20:1)));token.progress+=dt*speed*90/distance;if(token.progress>=1){token.nodeId=to.id;token.edge=null;token.progress=0;token.readyAt=s.t+cadDuration(to);s.movedItems++;if(to.id===s.asrs.equipmentId){s.asrs.inventory=Math.min(s.asrs.capacity,s.asrs.inventory+1);s.asrs.putaways++;this.emit('asrs-putaway',{equipmentId:to.id});}this.emit('equipment-start',{equipmentId:to.id});}}else if(s.t>=token.readyAt){const options=this.outgoing.get(token.nodeId)||[];if(options.length){if(token.nodeId===s.asrs.equipmentId){s.asrs.inventory=Math.max(0,s.asrs.inventory-1);s.asrs.retrievals++;this.emit('asrs-retrieval',{equipmentId:token.nodeId});}token.edge=options[(token.id-1)%options.length];token.progress=0;}else{s.completedProducts.push({id:token.id,cycleTime:s.t-token.createdAt});s.cadTokens.splice(s.cadTokens.indexOf(token),1);this.emit('equipment-complete',{equipmentId:token.nodeId,productId:token.id});}}}return s;}
