...
test('속도를 갖는 이동 설비는 가속도와 감속도를 함께 입력한다',()=>{for(const type of ['conveyor','sorter','forkingDevice','agv','amr','shuttle','forklift','lift','stackerCrane']){const keys=parameterFieldsFor({type,parameters:{}}).map(field=>field.key);assert.ok(keys.includes('acceleration'),`${type} acceleration`);assert.ok(keys.includes('deceleration'),`${type} deceleration`);}});
