...
function renderCadEquipmentParameters(selectedId=selectedEquipment?.id){
  ...
  const forks=items.filter(item=>item.type==='forkingDevice'),flowNames=Object.keys(engine.state?.asrs?.zones||{});if(active&&forks.length){const rule=document.createElement('div');rule.className='fork-routing-rule';const title=document.createElement('strong');title.textContent='자동 분배 조건 · 출발 라인별 독립 순차';const detail=document.createElement('span');detail.textContent=`${flowNames.join(' · ')||'DXF 인식 라인'} 각각: 첫 물류 직행 → 다음 물류 교차 분배 → 반복`;const ratio=document.createElement('small');ratio.textContent=`포킹장치의 직행 비율을 변경하면 각 라인에 독립 적용됩니다. 현재 ${forks.map(item=>`${item.name} ${Number(item.parameters?.output1Ratio??50)}:${100-Number(item.parameters?.output1Ratio??50)}`).join(' · ')}`;rule.append(title,detail,ratio);cadParameterSection.querySelector('.equipment-counts')?.after(rule);}
