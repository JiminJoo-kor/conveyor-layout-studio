...
export function commonParameterKeys(item, requested=null) {
  const allowed = new Set([...parameterFieldsFor(item).filter(field=>!field.readOnly).map(field=>field.key), 'stationConveyorsEnabled','infeedBufferCount','outfeedBufferCount','stationConveyorSpeed','stationSafetyGap','diverterAutoDynamics','autoMotionTuning','continuousHandover','handoverDelay','batchReleaseEnabled','releaseBatchSize','jerk','motionProfile','performance','efficiency','availability','mtbf','mttr','microStopInterval','microStopDuration']);
