...
function renderCadEquipmentParameters(selectedId=selectedEquipment?.id){
  ...
  const legacyConveyor=items.find(item=>item.type==='conveyor'&&(item.parameters?.cargoLength||item.parameters?.cargoWidth));layout.cargoSpec??={length:Number(legacyConveyor?.parameters?.cargoLength||1.2),width:Number(legacyConveyor?.parameters?.cargoWidth||.8)};
