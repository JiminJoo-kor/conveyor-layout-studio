  ...
  drawCadFlow(state){
    ...
    if(state.cadTokens){
      ...
      for(const token of state.cadTokens){
        ...
        const x=point.x,y=point.y;c.save();if(clipItem){const bounds=equipmentClipBounds(clipItem,this.layout.cadViewMode==='hybrid'?58:78),itemAngle=(Number(clipItem.rotation)||0)*Math.PI/180;c.translate(clipItem.x,clipItem.y);c.rotate(itemAngle);c.beginPath();c.rect(-bounds.width/2,-bounds.height/2,bounds.width,bounds.height);c.clip();c.rotate(-itemAngle);c.translate(-clipItem.x,-clipItem.y);}c.translate(x,y);c.rotate(cargoAngle);c.fillStyle=cargoColor(token.cargoType||token.flowKey,token.flowIndex);c.strokeStyle=flowColor(token.flowKey,token.flowIndex);c.lineWidth=2;c.fillRect(-cargoW/2,-cargoH/2,cargoW,cargoH);c.strokeRect(-cargoW/2,-cargoH/2,cargoW,cargoH);c.restore();
      }
      return;
    }
