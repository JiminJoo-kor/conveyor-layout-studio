...
import { asrsLayoutCargoPresentation, asrsLayoutCargoVisible, asrsOccupiedSlots, asrsRackCells, cargoColor, equipmentOperationProgress, equipmentVisualPosition, flowColor, flowDisplayTitle, isNodeConveyor, laneTitleAnchor, mobileEquipmentBridge, mobileEquipmentRoute, normalizedCargoSpec, shouldDrawCadToken } from '../src/renderer.js';
...
test('포킹장치와 AMR은 현장 동작 순서가 보이는 간단 파라미터만 제공한다',()=>{const forkKeys=parameterFieldsFor({type:'forkingDevice',parameters:{jerk:9}}).map(field=>field.key),amr={type:'amr',parameters:{motionProfile:1}},amrFields=parameterFieldsFor(amr),amrKeys=amrFields.map(field=>field.key),angle=amrFields.find(field=>field.key==='cargoTransferAngle');ensureDynamicParameters(amr);assert.ok(['strokeDistance','receiveSpeed','holdTime','transferSpeed'].every(key=>forkKeys.includes(key)));assert.ok(amrKeys.includes('receiveSpeed')&&amrKeys.includes('travelSpeed')&&amrKeys.includes('transferSpeed'));assert.equal(angle.label,'배출 물류 방향(°)');assert.equal(angle.value,0);assert.equal(amr.parameters.cargoTransferAngle,0);assert.equal(forkKeys.includes('jerk'),false);assert.equal(amrKeys.includes('motionProfile'),false);});
